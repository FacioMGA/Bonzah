...
// JSON configuration is not emitted by TypeScript either. The policy-list
// registry is loaded at process startup by both API and worker processes, so
// both production images must explicitly retain its canonical source folder.
const RUNTIME_SOURCE_DIRS_REQUIRING_DOCKER_COPY = [
  'frontend/src/modules/policies/list',
];

const STATIC_PDF_ROOT = 'backend';

export function productionStageText(dockerfileText) {
  const productionStage = /^FROM\s+\S+(?:\s+AS\s+production)\s*$/im.exec(dockerfileText);
  return productionStage?.index === undefined ? '' : dockerfileText.slice(productionStage.index);
}

export function missingProductionRuntimeSourceDirs(
  dockerfileText,
  sourceDirs = RUNTIME_SOURCE_DIRS_REQUIRING_DOCKER_COPY,
) {
  const production = productionStageText(dockerfileText);
  return sourceDirs.filter((sourceDir) => {
    const normalized = sourceDir.split(path.sep).join('/');
    return !production.includes(`COPY --from=backend-builder /app/${normalized} ./${normalized}`);
  });
}
...
function main() {
  ...
  for (const dockerfile of DOCKERFILES_REQUIRING_STATIC) {
    ...
    for (const sourceDir of missingProductionRuntimeSourceDirs(text)) {
      copyGaps.push(`${dockerfile}: no production-stage COPY for runtime source dir '${sourceDir}'`);
    }
