...
import { encodeTypeScriptStringLiteral, escapeMarkdownTableCell } from './generatedArtifactEscaping.js';
...
function stringifyStringMap(map: Record<string, string[]>, indent = 2): string {
  ...
  const lines = entries.map(([key, values]) => {
    const escapedKey = encodeTypeScriptStringLiteral(key);
    if (values.length === 0) return `${pad}${escapedKey}: [],`;
    const valueLines = values.map((v) => `${inner}${encodeTypeScriptStringLiteral(v)},`).join('\n');
...
function renderProductMapRegistry(): string {
  ...
  const rows = PRODUCT_MAP_REGISTRY.map((entry) => {
    return `| ${escapeMarkdownTableCell(entry.category)} | ${escapeMarkdownTableCell(entry.status)} | ${escapeMarkdownTableCell(entry.motor)} | ${escapeMarkdownTableCell(entry.home)} | ${escapeMarkdownTableCell(entry.travel)} | ${escapeMarkdownTableCell(entry.notes)} |`;
  });
...
function main(): void {
  ...
  // ---- Per-product non-validation maps (endorsement / BDX import) ----
