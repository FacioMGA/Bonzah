...
import { invokeTenantJob } from './platformJobContext.js';
...
const AUDIT_ONLY_EVENT_TYPES: ReadonlySet<string> = new Set([
    ...
    'PLATFORM.TENANT_PROFILE_UPDATED',
    'COMM.MESSAGE_SENT',
...
async function dispatchBehaviorFanout(eventType: string, payload: unknown, eventId: string | undefined): Promise<void> {
    if (process.env.KERNEL_PLATFORM_MODE === 'true' && process.env.KERNEL_BEHAVIOR_EMBEDDINGS_ENABLED !== 'true') return;
    try {
...
function enqueueOutboxEvent(
    ...
    if (!operatingTenantId && process.env.KERNEL_PLATFORM_MODE === 'true') {
        // A shared process has no default MGA. Enumerate active workspaces and
        // create an independent event identity for each durable tenant envelope.
        void (async () => {
            let cursor: string | undefined;
            for (;;) {
                const rows = await prisma.tenant.findMany({ where: { status: 'ACTIVE', parentOrganization: { active: true } },
                    orderBy: { id: 'asc' }, take: 100, select: { id: true }, ...(cursor ? { cursor: { id: cursor }, skip: 1 } : {}) });
                for (const row of rows) {
                    const eventId = randomUUID();
                    const payload = data.payload && typeof data.payload === 'object' && !Array.isArray(data.payload) ? data.payload : {};
                    enqueueOutboxEvent({ ...data, eventId, idempotencyKey: data.idempotencyKey ? `${row.id}:${data.idempotencyKey}` : null,
                        payload: { ...payload, eventId, operatingTenantId: row.id } }, row.id);
                }
                if (rows.length < 100) break;
                cursor = rows[rows.length - 1].id;
            }
        })().catch((err: unknown) => logger.error({ err }, 'queue.platform_schedule_failed'));
        return;
    }
...
function enqueueRenewalEmailScanForActiveProductionTenants(): void {
    const scheduledAt = new Date().toISOString();
    void prisma.tenant.findMany({ // guard:cross-tenant-intentional - scheduler enumerates active production tenants before restoring each explicit tenant context.
        where: { kind: 'PRODUCTION', status: 'ACTIVE' }, select: { id: true },
    }).then((tenants) => {
        for (const tenant of tenants) {
            const envelope = buildRenewalEmailScanEvent({ tenantId: tenant.id, scheduleKey: `hour:${scheduledAt.slice(0, 13)}`, scheduledAt });
            enqueueOutboxEvent({
                eventId: envelope.eventId, idempotencyKey: envelope.idempotencyKey || null,
                aggregateId: envelope.aggregateId, eventType: envelope.eventType,
                payload: envelope as Prisma.InputJsonValue,
            }, tenant.id);
        }
    }).catch((err: unknown) => logger.error({ event: 'queue.renewal_email_scan.schedule_failed', err }, 'queue.renewal_email_scan.schedule_failed'));
}
export function initWorkers() {
    ...
    if (queueEnabled('notifications')) {
      notificationsWorker = new Worker(QUEUE_NAMES.NOTIFICATIONS, async (job) => {
        ...
        if (handler) {
            return await invokeTenantJob(job, handler);
        }
    ...
    if (Number.isFinite(renewalScanMinutes) && renewalScanMinutes > 0) {
        ...
        setInterval(enqueueRenewalEmailScanForActiveProductionTenants, intervalMs);
    }
