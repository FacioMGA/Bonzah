...
describe('worker concurrency connection-pool budget', () => {
  ...
  it('schedules issued-pack recovery for every active production tenant', () => {
    ...
    expect(queueSource).toContain('tenantId: tenant.id');
    expect(queueSource).toContain('enqueueIssuedPackReconcileForActiveProductionTenants({');
  });

  it('schedules renewal scans for every active production tenant', () => {
    expect(queueSource).toContain('buildRenewalEmailScanEvent({');
    expect(queueSource).toContain('setInterval(enqueueRenewalEmailScanForActiveProductionTenants, intervalMs)');
    expect(queueSource).toContain('payload: envelope as Prisma.InputJsonValue');
    expect(queueSource).toContain('}, tenant.id);');
  });
});
