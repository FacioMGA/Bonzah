...
const TERMS_ATTACHMENTS: Readonly<Record<string, { filename: string; staticFilename: string }>> = {
  ...
  GR: {
    filename: 'TermsAndConditionsGreece.pdf',
    staticFilename: 'TermsAndConditions_CY.pdf',
  },
  PT: {
...
export function getTenantTermsAttachment(countryCode: string): CommunicationAttachmentRef | null {
  // Shared workspaces cannot acquire customer legal terms from their country.
  // Tenant-selected programme documents already enter through the explicit
  // message attachments. These archived source assets remain available only
  // to the original single-customer application, never this platform.
