...
vi.mock('../../../../platform/db/connection.js', async (importOriginal) => {
  ...
  const mockPrisma = {
    ...
    communicationDeliveryAttempt: {
      ...
      findMany: prismaMocks.deliveryAttemptFindMany,
...
describe('webhooksRouter inbound auth', () => {
  ...
  beforeEach(() => {
    ...
    prismaMocks.runTransaction.mockImplementation(async (fn: (tx: unknown) => Promise<void>) =>
      fn({
        communicationMessage: { create: prismaMocks.messageCreate, update: prismaMocks.messageUpdate, updateMany: prismaMocks.messageUpdateMany },
        communicationThread: { update: prismaMocks.threadUpdate },
        communicationDeliveryAttempt: { update: prismaMocks.deliveryAttemptUpdate, updateMany: prismaMocks.deliveryAttemptUpdateMany },
      }),
    );
    ...
    prismaMocks.deliveryAttemptFindMany.mockResolvedValue([]);
    prismaMocks.deliveryAttemptUpdate.mockResolvedValue({});
    prismaMocks.deliveryAttemptUpdateMany.mockResolvedValue({ count: 1 });
  ...
  async function postSendgridEvents(baseUrl: string, body: unknown[]) {
    ...
    return fetch(`${baseUrl}/webhooks/sendgrid/events`, {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
        'x-twilio-email-event-webhook-timestamp': timestamp,
        'x-twilio-email-event-webhook-signature': signSendgridEvent(body, timestamp, privateKey),
      },
  ...
  it('updates delivery attempt and message status from SendGrid event callbacks', async () => {
    await withServer(async (baseUrl) => {
      ...
      prismaMocks.deliveryAttemptFindMany.mockResolvedValueOnce([{
        id: 'attempt_sg_1',
        messageId: 'message_sg_1',
        attemptedAt: new Date('2026-04-07T10:58:55.000Z'),
        status: 'SENT',
      ...
      expect(response.status).toBe(200);
      expect(prismaMocks.deliveryAttemptFindMany).toHaveBeenCalledTimes(1);
      expect(prismaMocks.runTransaction).toHaveBeenCalledTimes(1);
      expect(prismaMocks.deliveryAttemptUpdateMany).toHaveBeenCalledWith(expect.objectContaining({
        where: expect.objectContaining({ id: 'attempt_sg_1' }),
        data: expect.objectContaining({
          status: 'DELIVERED',
          resolvedAt: expect.any(Date),
        }),
      }));
      expect(prismaMocks.messageUpdateMany).toHaveBeenCalledWith(expect.objectContaining({
        where: expect.objectContaining({ id: 'message_sg_1' }),
        data: expect.objectContaining({
          status: 'DELIVERED',
          deliveredAt: expect.any(Date),
        }),
  ...
  it('matches SendGrid dropped callbacks by the root x-message-id when custom args are missing', async () => {
    await withServer(async (baseUrl) => {
      ...
      prismaMocks.deliveryAttemptFindMany.mockResolvedValueOnce([{
        ...
        attemptedAt: new Date('2026-07-15T14:40:19.000Z'),
        status: 'SENT',
      ...
      expect(response.status).toBe(200);
      expect(prismaMocks.deliveryAttemptFindMany).toHaveBeenCalledWith(expect.objectContaining({
        where: expect.objectContaining({
          externalId: { in: ['root-sendgrid-id.recvd-656b4879fd-jrcm4-1-6A579BCD-6.0', 'root-sendgrid-id'] },
        }),
      }));
      expect(prismaMocks.deliveryAttemptUpdateMany).toHaveBeenCalledWith(expect.objectContaining({
        where: expect.objectContaining({ id: 'attempt_sg_drop' }),
        data: expect.objectContaining({
          status: 'FAILED',
      ...
      expect(prismaMocks.messageUpdateMany).toHaveBeenCalledWith(expect.objectContaining({
        where: expect.objectContaining({ id: 'message_sg_drop' }),
        data: expect.objectContaining({ status: 'FAILED' }),
      }));
    });
  });

  it.each(['delivered', 'bounce'])('keeps delayed %s evidence on the older attempt without projecting over a retry', async (event) => {
    await withServer(async (baseUrl) => {
      const attemptedAt = new Date('2026-09-01T10:00:00Z');
      prismaMocks.deliveryAttemptFindMany.mockResolvedValueOnce([{
        id: 'attempt-old', messageId: 'message-retried', attemptedAt,
        externalId: 'provider-old', status: 'SENT',
        message: { id: 'message-retried', status: 'DELIVERED', sentAt: attemptedAt, threadId: 'thread-retried' },
      }]);
      // Database relation predicate finds a newer (or tied) attempt.
      prismaMocks.messageUpdateMany.mockResolvedValueOnce({ count: 0 });
      const response = await postSendgridEvents(baseUrl, [{
        event, timestamp: 1788256860, sg_message_id: 'provider-old', communicationMessageId: 'message-retried',
      }]);

      expect(response.status).toBe(200);
      expect(prismaMocks.deliveryAttemptUpdateMany).toHaveBeenCalledWith(expect.objectContaining({
        where: expect.objectContaining({ id: 'attempt-old' }),
        data: expect.objectContaining({ status: event === 'bounce' ? 'BOUNCED' : 'DELIVERED', resolvedAt: new Date(1788256860 * 1000) }),
      }));
      expect(prismaMocks.messageUpdateMany).toHaveBeenCalledWith(expect.objectContaining({
        where: {
          id: 'message-retried',
          deliveryAttempts: { none: { provider: 'SENDGRID', channel: 'EMAIL', id: { not: 'attempt-old' }, attemptedAt: { gte: attemptedAt } } },
        },
      }));
      expect(prismaMocks.messageUpdate).not.toHaveBeenCalled();
      expect(prismaMocks.threadUpdate).not.toHaveBeenCalled();
    });
  });

  it.each([
    ['DELIVERED', 'processed'], ['DELIVERED', 'deferred'], ['BOUNCED', 'delivered'], ['FAILED', 'processed'],
  ])('preserves %s attempt evidence when a late %s callback arrives', async (priorStatus, event) => {
    await withServer(async (baseUrl) => {
      prismaMocks.deliveryAttemptFindMany.mockResolvedValueOnce([{
        id: 'attempt-terminal', messageId: 'message-terminal', status: priorStatus,
        attemptedAt: new Date('2026-09-01T10:00:00Z'),
        message: { id: 'message-terminal', status: priorStatus === 'DELIVERED' ? 'DELIVERED' : 'FAILED', sentAt: new Date(), threadId: 'thread-terminal' },
      }]);
      prismaMocks.deliveryAttemptUpdateMany.mockImplementationOnce(async (args: { where: { status: { in: string[] } } }) => ({
        count: args.where.status.in.includes(priorStatus) ? 1 : 0,
      }));
      const response = await postSendgridEvents(baseUrl, [{ event, timestamp: 1788256800, sg_message_id: 'provider-terminal' }]);

      expect(response.status).toBe(200);
      const update = prismaMocks.deliveryAttemptUpdateMany.mock.calls[0][0] as { where: { status: { in: string[] } } };
      expect(update.where.status.in).not.toContain(priorStatus);
      expect(prismaMocks.messageUpdateMany).not.toHaveBeenCalled();
      expect(prismaMocks.messageUpdate).not.toHaveBeenCalled();
      expect(prismaMocks.threadUpdate).not.toHaveBeenCalled();
    });
  });

  it('acknowledges an unmatched provider callback without mutating a known message', async () => {
    await withServer(async (baseUrl) => {
      prismaMocks.deliveryAttemptFindMany.mockResolvedValueOnce([]).mockResolvedValueOnce([{
        id: 'attempt-new', messageId: 'known-message', externalId: 'provider-new',
      }]);
      const response = await postSendgridEvents(baseUrl, [{ event: 'bounce', sg_message_id: 'provider-old', custom_args: { communicationMessageId: 'known-message' } }]);
      expect(response.status).toBe(200);
      expect(prismaMocks.runTransaction).not.toHaveBeenCalled();
    });
  });
