    ...
    try {
        ...
        if (replyIds.length > 0 && !threadId) {
            for (const refId of replyIds) {
                const matchedMsg = await tenantScopedPrisma.communicationMessage.findFirst({
                    where: {
                        externalRefs: {
                            path: ['messageId'],
                            equals: refId,
        ...
        if (!threadId && subject) {
            ...
            if (threadTagMatch?.[1]) {
                const taggedThread = await tenantScopedPrisma.communicationThread.findUnique({
        ...
        if (!threadId) {
            const recentThread = await tenantScopedPrisma.communicationThread.findFirst({
                where: {
                    messages: {
                        some: {
                            toRecipients: {
        ...
        if (!threadId) {
            ...
            const newThread = await tenantScopedPrisma.communicationThread.create({
                data: {
                    entityType: 'PARTY',
                    entityId: 'UNKNOWN',
                }
...
router.post('/twilio/status', async (req, res) => {
    try {
        ...
        const attempt = await tenantScopedPrisma.communicationDeliveryAttempt.findFirst({
...
router.post('/sendgrid/events', async (req, res) => {
    try {
        ...
        for (const event of parsed.data) {
            ...
            await runTenantScopedTransaction(async (_tx) => {
              const tx = _tx as unknown as Prisma.TransactionClient;
                const updatedAttempt = await tx.communicationDeliveryAttempt.updateMany({
                    where: {
                        id: attempt.id,
                        // Use the existing message-state priority for attempts
                        // too; BOUNCED maps to FAILED and remains terminal.
                        status: { in: ['QUEUED', ...(['SENDING', 'SENT', 'DELIVERED', 'BOUNCED', 'FAILED'] as const)
                            .filter((status) => statusPriority(mapAttemptStatusToMessageStatus(status)) <= statusPriority(nextMessageStatus))] },
                    },
                    data: {
                ...
                if (updatedAttempt.count !== 1) return;
                ...
                const updatedMessage = await tx.communicationMessage.updateMany({
                    where: {
                        id: attempt.messageId,
                        // A delayed event still belongs to its exact attempt,
                        // but must not project over a newer (or tied) retry.
                        deliveryAttempts: { none: {
                            provider: 'SENDGRID',
                            channel: 'EMAIL',
                            id: { not: attempt.id },
                            attemptedAt: { gte: attempt.attemptedAt },
                        } },
                    },
                    data: messageData,
                });
                if (updatedMessage.count !== 1) return;
                await tx.communicationThread.update({
                    where: { id: attempt.message.threadId },
