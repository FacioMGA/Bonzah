 ...
 * fail-closed extension). The mock therefore exposes both clients but the canonical
 * `runTenantScopedTransaction` boundary is exercised by these tests.
 *
...
function bindTransactionMock(mockTx: MockTransactionClient): void {
  const txFn = runTenantScopedTransaction as unknown as {
