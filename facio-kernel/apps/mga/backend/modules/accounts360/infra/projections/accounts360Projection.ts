...
export async function rebuildAccount360Projection(accountId: string) {
  ...
  if (!holder) {
    await runTenantScopedTransaction(async (tx) => {
      await tx.accountSummaryProjection.deleteMany({ where: { accountId: id } });
      await tx.accountPortfolioMetrics.deleteMany({ where: { accountId: id } });
  ...
  const communicationThreads = threadEntityOr.length
    ? await tenantScopedPrisma.communicationThread.findMany({
