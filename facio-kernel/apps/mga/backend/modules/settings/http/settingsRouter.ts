...
router.get('/global_commissions', async (_req: Request, res: Response) => {
    try {
        ...
        let setting = await tenantScopedPrisma.settings.findFirst({
...
router.post('/global_commissions', async (req: Request, res: Response) => {
    try {
        ...
        const existing = await tenantScopedPrisma.settings.findFirst({
            orderBy: { updatedAt: 'desc' },
        });

        let setting;
        if (existing) {
            setting = await tenantScopedPrisma.settings.update({
            ...
            setting = await tenantScopedPrisma.settings.create({
                data: {
                    defaultRate: new Prisma.Decimal(rate),
                    commissions: JSON.stringify(splits)
                }
...
router.get('/templates', async (_req: Request, res: Response) => {
    try {
        ...
        const setting = await tenantScopedPrisma.settings.findFirst({
            orderBy: { updatedAt: 'desc' },
        });
...
router.post('/templates', async (req: Request, res: Response) => {
    try {
        ...
        const existing = await tenantScopedPrisma.settings.findFirst({ orderBy: { updatedAt: 'desc' } });
...
router.post('/templates/upload', upload.single('file'), async (req: Request, res: Response) => {
    try {
        ...
        if (existing) {
            ...
            await tenantScopedPrisma.settings.update({
                where: { id: existing.id },
                data: updateData
            });
        } else {
            ...
            await tenantScopedPrisma.settings.create({ data: createData });
        }
