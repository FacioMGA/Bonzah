// Reused from Facio Kernel src/server/oidc.ts (0dbbfa0); storage-independent OIDC verification.
import { createRemoteJWKSet, jwtVerify } from 'jose';
import { createHash, randomUUID } from 'node:crypto';
import { z } from 'zod';
import { digest, secret, IdentityError as KernelError } from './identity.js';
...
export function oidcProvider(options: OidcOptions): IdentityProvider {
  ...
  return {
    ...
    async callback(code, flow) {
      ...
      if (microsoft && (payload.tid !== options.tenantId || !z.string().uuid().safeParse(payload.oid).success))
