...
export async function authenticateMcpRequest(
    ...
    // Shared Platform has already verified the signed tenant session and
    // re-read its active organization and tenant memberships on this request.
    // Keep permission resolution in this canonical MCP admission funnel.
    if (process.env.KERNEL_PLATFORM_MODE === 'true' && req.user?.platformTenantAuthorized === true) {
        req.resolvedPermissions = await resolveEffectivePermissionsForUser(req.user.id, req.user.role);
        next();
        return;
    }
...
async function validateFacioBranch(
    ...
    try {
        ...
        if (validated.operatingTenant.id !== resolvedTenant.id || validated.account.operatingTenantId !== resolvedTenant.id) {
            logger.warn(
                {
