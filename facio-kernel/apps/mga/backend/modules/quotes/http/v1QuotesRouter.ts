...
import { buildQuoteResponseForProduct, productAdapterExists, resolvePublishedProgramQuoteContext } from '../../policy/app/productRegistryService.js';
...
router.post('/', async (req, res) => {
    try {
            ...
            include: {
                binder: {
                    select: {
                        id: true,
                        productAuthorities: {
                            where: { productCode: resolvedProductType, status: 'ACTIVE' },
                            select: { id: true },
                            take: 1,
                        },
                    },
        ...
        const paramsByCode = parsed.data.endorsements?.coverageSelection?.params as Prisma.JsonObject | undefined;
        ...
        const binderProductAuthorityId = activeBinderLink?.binder.productAuthorities[0]?.id || null;
        if (!binderProductAuthorityId) {
            return res.status(409).json({ success: false, error: { message: 'No active binder authority is mapped to this programme.' } });
        }
        ...
        return res.status(201).json({
            ...
            data: {
                ...
                endorsements: { applied: programmeContext.resolvedCoverageSet?.applied ?? [] },
