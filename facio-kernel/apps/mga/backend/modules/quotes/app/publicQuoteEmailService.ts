...
function buildIpidAttachment(productType: string, countryCode: string, quoteData: unknown): CommunicationAttachmentRef {
  const asset = resolveProductIpidAsset(productType, countryCode, { quoteData });
...
export async function sendPublicQuoteEmailForSession(
  ...
  if (!packIncludesIpid) {
    try {
      extraAttachments = [buildIpidAttachment(productCode, getTenantConfig().countryCode, quoteData)];
    } catch (error) {
