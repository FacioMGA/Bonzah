...
router.get('/:token/context', async (req, res) => {
  try {
    ...
    if (!claim.policy) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Policy is unavailable' } });
    await assertPolicyJourneyAction(claim.policy, 'customer', 'claim');
    ...
    if (error instanceof InsuranceConfigurationError) return res.status(403).json({ success: false, error: { code: error.code, message: error.message } });
    if (error instanceof ProgramDefinitionConfigurationError) return res.status(503).json({ success: false, error: { code: error.code, message: error.message } });
...
router.post('/:token/submit', async (req, res) => {
  try {
    ...
    const claim = await tenantScopedPrisma.claim.findUnique({
      where: { id: claimId },
      select: { id: true, policyId: true, status: true, policy: { select: { programId: true, binderId: true, productType: true } } },
    });
    if (!claim || !claim.policyId || (policyId && claim.policyId !== policyId)) {
      return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'FNOL link is invalid or no longer available' } });
    }
