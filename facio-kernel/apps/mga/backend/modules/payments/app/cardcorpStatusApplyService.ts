...
export async function applyCardcorpVerifiedStatus(args: {
  ...
  jsonStringify: (value: unknown) => Prisma.InputJsonValue;
  resolveInceptionDateFromRenewalDate: (renewalDateRaw: unknown) => Date;
  resolveTenantId: () => string;
}): Promise<{
  updatedPayment: { id: string; status: string };
  selectedBundleId: string | null;
  awaitingExternalIssuance?: boolean;
  ...
  const updatedPayment = await runTenantScopedTransaction(async (_tx) => {
    const tx = _tx as unknown as Prisma.TransactionClient;
    const current = await tx.payment.findUnique({
      where: { id: args.payment.id },
      select: { id: true, status: true },
    });
    // A re-rate can cancel a checkout between provider verification and this
    // apply transaction. Cancellation is terminal: never resurrect that
    // stale provider result into a paid policy or issuance attempt.
  ...
  if (!updatedPayment) {
    return {
      updatedPayment: { id: args.payment.id, status: 'CANCELLED' },
  ...
  let awaitingExternalIssuance = false;
  if (args.status.ok) {
    ...
    awaitingExternalIssuance = Boolean(issuance.awaitingExternalIssuance);
  ...
  return {
    updatedPayment,
    selectedBundleId,
    ...(awaitingExternalIssuance ? { awaitingExternalIssuance: true } : {}),
    outcome: 'APPLIED',
  };
}
