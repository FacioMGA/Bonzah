...
import { resolvePolicyProgramDocumentConfiguration } from '../../policy/app/productRegistryService.js';
import {
  ...
  const work = (async (): Promise<HealResult> => {
    try {
      ...
      const policy = await tenantScopedPrisma.policy.findUnique({
        where: { id: policyId },
        select: { status: true, productType: true, programId: true, binderId: true },
      });
      if (String(policy?.status || '').trim().toUpperCase() === 'AWAITING_EXTERNAL_ISSUANCE') {
        return { healed: false, reason: 'external_issuance_pending' };
      }
      ...
      if (inception) {
        const documentConfiguration = await resolvePolicyProgramDocumentConfiguration({
          productType: String(policy?.productType || args.policy.productType || '').trim().toUpperCase(),
          programId: String(policy?.programId || ''),
          binderId: String(policy?.binderId || ''),
        });
        const requiredDocTypes = documentConfiguration.requiredIssuedDocTypes;
...
export async function reconcileCardcorpPaidIssuanceBatch(
  ...
  const candidates = await tenantScopedPrisma.policy.findMany({
    where: {
      ...
      status: { not: 'AWAITING_EXTERNAL_ISSUANCE' },
      OR: [
        {
