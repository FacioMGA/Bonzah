 ...
 *
 * The actual ground-truth signal that the payment spine completed is
 * an `INCEPTION` `RiskTransaction`, except for a product adapter that
 * explicitly declares ADR-0096 external issuance. That authorised
 * paid state is persisted as `AWAITING_EXTERNAL_ISSUANCE`; it must not
 * be retried as automatic issuance.
 */
async function policyHasCompletedCardcorpPaymentSpine(policyId: string): Promise<boolean> {
  ...
  return String(policy?.status || '').trim().toUpperCase() === 'AWAITING_EXTERNAL_ISSUANCE';
}
...
> {
  ...
  if (payment.status === 'CANCELLED') {
    return {
      ...
      data: {
        ...
        description: 'This payment session was cancelled after the quote changed. Please restart payment from the refreshed quote.',
  ...
  if ((payment.status === 'PAID' || payment.status === 'FAILED') && storedResult.code) {
    ...
    if (ok && payment.status === 'PAID') {
      const spineRan = await policyHasCompletedCardcorpPaymentSpine(args.policyId);
      if (!spineRan) {
        logger.warn(
