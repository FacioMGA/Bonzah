...
const resolvePolicyProgramDocumentConfigurationMock = vi.fn(async () => ({
...
vi.mock('../../../policy/app/productRegistryService.js', () => ({
  resolvePolicyProgramDocumentConfiguration: (...args: unknown[]) =>
    resolvePolicyProgramDocumentConfigurationMock(...args),
}));
...
describe('cardcorpIssuanceHealService.attemptIssuanceHealForPolicy', () => {
  beforeEach(async () => {
    vi.resetAllMocks();
    resolvePolicyProgramDocumentConfigurationMock.mockResolvedValue({
      requiredIssuedDocTypes: ['MOTOR_CERTIFICATE_PDF'],
    });
    enqueueIssuedPolicyPackStandaloneMock.mockResolvedValue({ eventId: 'evt-1' });
    prismaMock.policy.findUnique.mockResolvedValue({
      status: 'AWAITING_PAYMENT', productType: 'MOTOR', programId: 'program-1', binderId: 'binder-1',
    });
  ...
  it('does not retry automatic issuance for a paid external-issuance policy', async () => {
    prismaMock.payment.findFirst.mockResolvedValueOnce({
      id: 'pay-1', paymentId: 'g-1', status: 'PAID', raw: {}, checkoutId: 'chk-1',
    });
    ...
    expect(result).toEqual({ healed: false, reason: 'external_issuance_pending' });
    expect(runCardcorpPaidIssuanceMock).not.toHaveBeenCalled();
  });
