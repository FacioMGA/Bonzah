...
const mocks = vi.hoisted(() => ({
  ...
  resolveProgrammeExternalIssuanceWorkflow: vi.fn(),
...
vi.mock('../../../../platform/db/connection.js', () => ({
  runTenantScopedTransaction: mocks.transaction,
  tenantScopedPrisma: {
...
vi.mock('../../../policy/app/productRegistryService.js', () => ({
  resolveProgrammeExternalIssuanceWorkflow: (...args: unknown[]) => mocks.resolveProgrammeExternalIssuanceWorkflow(...args),
}));

vi.mock('../../../../platform/events/domainEvents.js', () => ({
  appendDomainEvent: mocks.appendDomainEvent,
  buildDomainEvent: (event: unknown) => event,
}));
...
function buildCandidate(productType: string) {
  return {
    ...
    programId: `program-${productType.toLowerCase()}`,
...
describe('runCardcorpPaidIssuance product spine coverage', () => {
  beforeEach(() => {
    ...
    mocks.resolveProgrammeExternalIssuanceWorkflow.mockResolvedValue(null);
  ...
  it.each(['MOTOR', 'HOME', 'TRAVEL', 'HEALTH'])(
    ...
    async (productType) => {
      ...
      expect(mocks.resolveProgrammeExternalIssuanceWorkflow).toHaveBeenCalledWith({
        programId: candidate.programId,
        binderId: candidate.binderId,
        productType,
      });
  ...
  it('holds Open Market payment in external issuance until a manager uploads insurer documents', async () => {
    const candidate = {
      ...buildCandidate('OPEN_MARKET'),
      status: 'AWAITING_PAYMENT',
      quoteData: {
        proposer: { firstName: 'Ada', lastName: 'Market', email: 'ada@example.com' },
        renewalDate: '2026-05-09',
        manualPremium: 123.45,
        proposal: { marketName: 'Open Market', coverageRows: [{ premium: 123.45 }] },
      },
    };
    mocks.txPolicyFindUnique.mockResolvedValueOnce(candidate);
    mocks.resolveProgrammeExternalIssuanceWorkflow.mockResolvedValue({
      mode: 'manager_upload_after_payment',
      documentTypes: ['OPEN_MARKET_EXTERNAL_CERTIFICATE_PDF'],
    });
    ...
    const result = await runCardcorpPaidIssuance({
      ...
      status: { paymentId: 'gateway-payment-1', amount: '123.45', currency: 'EUR' },
      checkoutId: 'checkout-1',
      correlationId: 'corr-open-market-external',
      baseUrl: '',
      parseRecord,
      jsonStringify,
      resolveInceptionDateFromRenewalDate: () => new Date('2026-05-09T00:00:00.000Z'),
    ...
    expect(result).toEqual({ selectedBundleId: 'manual-open-market', awaitingExternalIssuance: true });
    expect(mocks.resolveProgrammeExternalIssuanceWorkflow).toHaveBeenCalledWith({
      programId: candidate.programId,
      binderId: candidate.binderId,
      productType: 'OPEN_MARKET',
    });
    expect(mocks.txRiskTransactionCreate).not.toHaveBeenCalled();
    expect(mocks.enqueueIssuedPolicyPack).not.toHaveBeenCalled();
    expect(mocks.transitionPolicyLifecycle).toHaveBeenCalledWith(expect.objectContaining({
      to: 'AWAITING_EXTERNAL_ISSUANCE',
      reasonCode: 'PAYMENT_CONFIRMED_EXTERNAL_ISSUANCE_REQUIRED',
    }));
    expect(mocks.appendDomainEvent).toHaveBeenCalledWith(tx, expect.objectContaining({
      eventType: 'EMAIL.UW_REFERRAL',
      data: expect.objectContaining({ policyId: candidate.id }),
    }));
  });
});
