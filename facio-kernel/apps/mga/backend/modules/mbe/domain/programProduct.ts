...
export function normalizeProgramMbeProductConfig(
  ...
  // stored flag) fire. This legacy normalizer is migration/test-only; a
  // published definition is parsed strictly and never self-heals at runtime.
...
/**
 * Validates a coverage component that has already been published as part of a
 * programme definition. Unlike the legacy normalizer this never infers a
 * programme code, fills missing arrays, filters unknown templates, or appends
 * templates introduced later. A published definition is the authority: an
 * incomplete or stale component must stop the quote instead of silently
 * changing cover.
 */
export function parsePublishedProgramMbeProductConfig(
  raw: unknown,
  opts: { productType: string; catalog?: EndorsementCatalog },
): ProgramMbeProductConfigV1 {
  if (!isObject(raw)) throw new Error('Published programme coverage must be an object.');
  if (raw.schemaVersion !== 1) throw new Error('Published programme coverage requires schemaVersion 1.');
  const programCode = typeof raw.programCode === 'string' ? raw.programCode.trim() : '';
  if (!programCode) throw new Error('Published programme coverage requires programCode.');

  const productType = String(opts.productType || '').trim().toUpperCase();
  if (!productType) throw new Error('Published programme coverage requires product type.');
  const inferredProductType = inferProductTypeFromProgramCode(programCode);
  if (inferredProductType && inferredProductType !== productType) {
    throw new Error(`Published programme coverage ${programCode} does not belong to ${productType}.`);
  }
  const catalog = resolveCatalog({ productType, programCode, catalog: opts.catalog });
  const validCodes = new Set(
    catalog.getAll()
      .filter((template) => String(template.program_code || '').trim() === programCode)
      .map((template) => String(template.code || '').trim())
      .filter(Boolean),
  );
  if (validCodes.size === 0) throw new Error(`Published programme coverage ${programCode} has no catalog templates.`);

  const parseEntries = (
    value: unknown,
    flag: 'enabled' | 'enabledByDefault',
  ): Array<{ code: string; [key: string]: boolean | string | UnknownRecord }> => {
    if (!Array.isArray(value)) throw new Error(`Published programme coverage requires ${flag === 'enabled' ? 'base' : 'options'} array.`);
    const seen = new Set<string>();
    return value.map((entry) => {
      if (!isObject(entry)) throw new Error('Published programme coverage entries must be objects.');
      const code = typeof entry.code === 'string' ? entry.code.trim() : '';
      if (!code || !validCodes.has(code)) throw new Error(`Published programme coverage contains unknown template ${code || '(missing)'}.`);
      if (seen.has(code)) throw new Error(`Published programme coverage duplicates template ${code}.`);
      seen.add(code);
      if (typeof entry[flag] !== 'boolean') throw new Error(`Published programme coverage ${code} requires boolean ${flag}.`);
      if (entry.params !== undefined && !isObject(entry.params)) throw new Error(`Published programme coverage ${code} params must be an object.`);
      return { code, [flag]: entry[flag], params: entry.params || {} };
    });
  };

  const base = parseEntries(raw.base, 'enabled').map((entry) => ({
    code: String(entry.code), enabled: Boolean(entry.enabled), params: entry.params as UnknownRecord,
  }));
  const options = parseEntries(raw.options, 'enabledByDefault').map((entry) => ({
    code: String(entry.code), enabledByDefault: Boolean(entry.enabledByDefault), params: entry.params as UnknownRecord,
  }));
  const duplicated = base.find((entry) => options.some((option) => option.code === entry.code));
  if (duplicated) throw new Error(`Published programme coverage places ${duplicated.code} in both base and options.`);
  return { schemaVersion: 1, programCode, base, options };
}
