...
import { resolveProgrammeCoverageConfiguration } from '../../policy/app/resolveProgrammeCoverageConfiguration.js';
...
import { resolveEndorsementRatingContext } from '../app/endorsementRatingContext.js';
...
export class MagicBService {
    ...
    static async previewEndorsement(policyId: string, endorsementCode: string, params: Record<string, unknown>, targetId?: string) {
        ...
        const currentSnapshot = currentState && typeof currentState.snapshot === 'object'
            ...
            : {};
        const cfg = programId
            ? await resolveProgrammeCoverageConfiguration({
                programId,
                binderId: String((policy as { binderId?: unknown }).binderId || ''),
                productType: String((policy as { productType?: unknown }).productType || ''),
            })
            : null;
        ...
        const policyProgramId = String((policy as { programId?: unknown }).programId || '').trim();
        const policyBinderId = String((policy as { binderId?: unknown }).binderId || '').trim();
        const ratingContext = await resolveEndorsementRatingContext({
            productType,
            programId: policyProgramId,
            binderId: policyBinderId,
            requiresProgramRatingModel: runtime?.rating.source === 'program_model',
        });

        const oldCalc = await adapter.calculateEndorsementPremium(quoteData, existingForCalc, ratingContext);
        const newCalc = await adapter.calculateEndorsementPremium(quoteData, proposedForCalc, ratingContext);
