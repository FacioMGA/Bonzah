...
import {
  buildDefaultProgramMbeProductConfig,
  normalizeProgramMbeProductConfig,
  parsePublishedProgramMbeProductConfig,
  resolveAppliedEndorsementsForQuote,
} from '../programProduct.js';
...
describe('program MBE product defaults', () => {
  it('rejects incomplete published coverage instead of defaulting it', () => {
    expect(() => parsePublishedProgramMbeProductConfig({}, { productType: 'MOTOR' }))
      .toThrow('schemaVersion 1');
    expect(() => parsePublishedProgramMbeProductConfig({
      schemaVersion: 1,
      programCode: 'abbeygate_motor',
      base: [],
    }, { productType: 'MOTOR' })).toThrow('requires options array');
  });

  it('accepts an explicit complete published coverage component', () => {
    const configured = buildDefaultProgramMbeProductConfig({ productType: 'MOTOR' });
    expect(parsePublishedProgramMbeProductConfig(configured, { productType: 'MOTOR' })).toEqual(configured);
  });
