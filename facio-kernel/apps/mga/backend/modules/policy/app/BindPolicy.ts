...
export async function executeBindPolicy(input: BindPolicyInput): Promise<UseCaseResult<Record<string, unknown>>> {
    ...
    try { await assertConfiguredBindingActor(candidate, actor); } catch (error) {
    ...
    try {
        const result = await runTenantScopedTransaction(async (_tx) => {
          const tx = _tx as unknown as Prisma.TransactionClient;
            ...
            const assigned = process.env.KERNEL_PLATFORM_MODE === 'true' ? await assignPlatformIssuanceIdentifiers(tx, id, 'MANUAL') : null;
            const policyBusinessId = assigned?.policyNumber ?? (shouldReassignPolicyNumberAtIssuance(productType, existingPolicyNumber)
