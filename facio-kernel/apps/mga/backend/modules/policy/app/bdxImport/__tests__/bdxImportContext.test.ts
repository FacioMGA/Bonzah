...
const { programFindUnique, programFindMany, binderFindUnique, binderProductAuthorityFindUnique, programBinderLinkFindFirst, programBinderLinkFindMany } = vi.hoisted(() => ({
  programFindUnique: vi.fn(),
  programFindMany: vi.fn(),
  binderFindUnique: vi.fn(),
  binderProductAuthorityFindUnique: vi.fn(),
...
vi.mock('../../../../../platform/db/connection.js', () => ({
  ...
  tenantScopedPrisma: {
    ...
    binderProductAuthority: {
      findUnique: binderProductAuthorityFindUnique,
    },
  },
}));
...
beforeEach(() => {
  ...
  binderProductAuthorityFindUnique.mockReset();
...
describe('resolveProgramBinder', () => {
  ...
  it('resolves the exact binder-product authority for BDX rating', async () => {
    binderProductAuthorityFindUnique.mockResolvedValueOnce({ id: 'authority-motor' });

    await expect(resolveBdxBinderProductAuthorityId({
      binderId: MOTOR_BINDER.id,
      productType: 'motor',
    })).resolves.toBe('authority-motor');
    expect(binderProductAuthorityFindUnique).toHaveBeenCalledWith({
      where: { binderId_productCode: { binderId: MOTOR_BINDER.id, productCode: 'MOTOR' } },
      select: { id: true },
    });
  });

  it('resolves each historical BDX row through its date-valid binder authority', async () => {
    binderProductAuthorityFindUnique.mockImplementation(async ({ where }: {
      where: { binderId_productCode: { binderId: string; productCode: string } };
    }) => ({ id: `authority-${where.binderId_productCode.binderId}` }));

    await expect(resolveBdxBinderProductAuthorityForEvaluation({
      dto: { inceptionDate: '2025-07-01T00:00:00.000Z', bookedDate: '' },
      binders: [TRAVEL_2025_BINDER, TRAVEL_2024_BINDER],
      productType: 'TRAVEL',
    })).resolves.toBe(`authority-${TRAVEL_2024_BINDER.id}`);

    expect(binderProductAuthorityFindUnique).toHaveBeenCalledWith({
      where: {
        binderId_productCode: {
          binderId: TRAVEL_2024_BINDER.id,
          productCode: 'TRAVEL',
        },
      },
      select: { id: true },
