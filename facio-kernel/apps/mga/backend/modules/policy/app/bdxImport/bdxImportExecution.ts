...
/**
 * BDX is an operational entry point, not an exception to programme
 * authority. Imported and renewed policies must re-rate using the exact
 * published definition mapped to their binder product authority.
 */
async function rateImportedQuote(args: {
  productType: string;
  quoteData: unknown;
  programId: string;
  binderId: string;
  ...
  if (!binderProductAuthorityId) {
    throw new Error(`BDX rating requires an active ${args.productType} binder product authority.`);
  }
  ...
  return buildQuoteResponseForProduct({
    productType: args.productType,
    quoteData: args.quoteData,
    programId: args.programId,
    binderProductAuthorityId,
...
export async function importPassingRow(args: BdxPolicyRowImportCommand): Promise<{
  ...
  const { quoteResponse, underwritingAnalysis } = await rateImportedQuote({
    productType,
    quoteData,
    programId,
    binderId,
...
export async function importRenewalTermRow(args: BdxRenewalTermImportCommand): Promise<{
  ...
  if (!priorPolicy.programId) return { error: 'Prior policy has no programme; cannot re-rate renewal without a published programme definition' };
  ...
  const renewalResult = await rateImportedQuote({
    productType: renewalProductType,
    quoteData,
    programId: priorPolicy.programId,
    binderId,
  });
