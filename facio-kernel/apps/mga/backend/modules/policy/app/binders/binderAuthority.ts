...
import { tenantScopedPrisma as defaultPrisma, runTenantScopedTransaction } from '../../../../platform/db/connection.js';
...
export async function assertBinderAuthorizesProduct(args: {
  ...
  const db: PrismaLike = args.db ?? (defaultPrisma as unknown as PrismaLike);
...
export async function findLatestActiveBinderLinkForProduct(args: {
  ...
  // Join models do not themselves carry a tenant key. Their protected parents
  // must be read on the same connection with the authorized tenant GUC set.
  if (!args.db) return runTenantScopedTransaction((tx) => findLatestActiveBinderLinkForProduct({ ...args, db: tx as unknown as PrismaLike }));
  ...
  const links = await db.programBinderLink.findMany({
    ...
    include: {
      binder: {
        select: {
          id: true, agreementNumber: true, umr: true, startDate: true, endDate: true,
          productAuthorities: {
            where: {
              productCode,
              status: 'ACTIVE',
  ...
  const [binderProductAuthority] = selected.binder.productAuthorities;
  if (!binderProductAuthority) return null;
  return {
    id: selected.id,
    programId: selected.programId,
    binderId: selected.binderId,
    program: selected.program,
    binder: {
      id: selected.binder.id,
      agreementNumber: selected.binder.agreementNumber,
      umr: selected.binder.umr,
      startDate: selected.binder.startDate,
      endDate: selected.binder.endDate,
    },
    binderProductAuthority,
  };
}
