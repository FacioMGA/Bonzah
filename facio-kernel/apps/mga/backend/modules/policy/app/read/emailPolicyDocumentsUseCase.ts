...
export type EmailPolicyDocumentsInput = {
  ...
  tenantId: string;
  documentIds: string[];
  /** Fail instead of sending a partial attachment set. */
...
export async function emailPolicyDocumentsUseCase(
  ...
  if (input.requireAllDocuments && docs.length !== uniqueIds.length) {
    return {
      status: 409,
      body: { success: false, error: { code: 'NOT_READY', message: 'Every requested document must be available before this email can be sent' } },
    };
  }
  ...
  if (input.requireAllDocuments && attachments.length !== uniqueIds.length) {
    return {
      status: 409,
      body: { success: false, error: { code: 'NOT_READY', message: 'Every requested document must be available before this email can be sent' } },
    };
