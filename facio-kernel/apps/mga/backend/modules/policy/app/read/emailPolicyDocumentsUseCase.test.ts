...
describe('emailPolicyDocumentsUseCase', () => {
  ...
  it('fails closed for the external issuance email when any requested attachment is unavailable', async () => {
    const deps = buildDeps({
      repo: {
        ...buildDeps().repo,
        findDocuments: vi.fn(async () => [
          {
            ...
            type: 'OPEN_MARKET_EXTERNAL_CERTIFICATE_PDF',
            docPack: 'ISSUED_POLICY_PACK',
            status: 'GENERATED',
          },
          {
            ...
            storageUri: '/api/documents/wording.pdf',
            type: 'OPEN_MARKET_EXTERNAL_POLICY_WORDING_PDF',
            docPack: 'ISSUED_POLICY_PACK',
            status: 'GENERATED',
          },
        ]),
      ...
      storage: {
        fetchPdfBufferFromStorageUri: vi.fn(async (uri: string) => uri.includes('certificate') ? Buffer.from('pdf-content') : null),
      },
      notifications: { sendRequestedDocumentsEmail: vi.fn(async () => true) },
    });

    const result = await emailPolicyDocumentsUseCase(
      { policyId: 'pol_1', tenantId: 'tenant_1', documentIds: ['doc_1', 'doc_2'], requireAllDocuments: true },
      deps,
    );

    expect(result.status).toBe(409);
    expect(result.body).toMatchObject({ error: { code: 'NOT_READY' } });
    expect(deps.notifications.sendRequestedDocumentsEmail).not.toHaveBeenCalled();
  });
});
