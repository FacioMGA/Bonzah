...
import { resolveImmutablePolicyDocumentConfiguration } from './productRegistryService.js';
...
export async function executeIssueEndorsement(
    ...
    try {
        ...
        const policyForDocs = await tenantScopedPrisma.policy.findUnique({
            where: { id: policyId },
            select: { productType: true, programId: true, binderId: true },
        });
        if (!policyForDocs) {
            throw new Error('Policy no longer exists while resolving endorsement documents.');
        }
        const documentConfiguration = await resolveImmutablePolicyDocumentConfiguration({
            policyId,
            riskTransactionId,
        });
        const requiredIssuedDocTypesForEmail = documentConfiguration.requiredIssuedDocTypes;
        ...
        const missingIssuedPackTypes = await getMissingIssuedPackTypes(
            policyId,
            requiredIssuedDocTypesForEmail,
        );
