...
import { buildQuoteResponseForProduct, productAdapterExists, resolvePublishedProgramQuoteContext } from './productRegistryService.js';
    ...
    try {
        ...
        const result = await runTenantScopedTransaction(async (_tx) => {
          const tx = _tx as unknown as Prisma.TransactionClient;
            ...
            if (!productAdapterExists(productType)) throw new Error(`No product adapter for: ${productType}`);
            const binderProductAuthorityId = policy.binderId
                ...
                : null;
            if (!policy.programId || !binderProductAuthorityId) {
                throw new Error('Endorsement rating requires the policy programme and active binder authority.');
            }
            ...
            const resolvedCoverageSet = programmeContext.coverage
                ? resolveEffectiveCoverageContract({
                    productType,
                    quoteData,
                    cfg: programmeContext.coverage,
                    storedSelection: prevDraft.coverageSelection,
                    programId: policy.programId,
                    source: isRenewal ? 'RENEWAL_DRAFT' : 'ENDORSEMENT_DRAFT',
                }).resolvedCoverageSet
                : undefined;
            const adapterResult = await buildQuoteResponseForProduct({
                productType,
                quoteData,
                programId: policy.programId,
                binderProductAuthorityId,
