...
describe('BindCoverage use-case spine', () => {
  ...
  it('rejects generic binding while external insurer documents are required', async () => {
    const { executeBindCoverage } = await import('../BindCoverage.js');
    prismaMock.policy.findUnique.mockResolvedValueOnce({
      id: 'pol-external',
    ...
    const result = await executeBindCoverage({
      policyId: 'pol-external',
      actor: { id: 'u1', name: 'Tester', email: 't@example.com', role: 'UNDERWRITER' },
    });

    expect(result).toMatchObject({
      status: 'INVALID_STATUS',
      error: { code: 'EXTERNAL_ISSUANCE_COMPLETION_REQUIRED' },
    });
    expect(transitionPolicyLifecycle).not.toHaveBeenCalled();
    expect(txMock.riskTransaction.create).not.toHaveBeenCalled();
  });
});
