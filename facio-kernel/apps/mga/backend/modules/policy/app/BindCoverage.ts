import { assertConfiguredBindingActor } from '../../insuranceConfiguration/app/journeyCapabilities.js';
...
export interface BindCoverageInput {
    ...
    /**
     * ADR-0096: only the external-issuance completion command may bind a
     * paid policy in this state, after it has validated the declared insurer
     * document pack. HTTP bind routes never set this capability.
     */
...
export async function executeBindCoverage(input: BindCoverageInput): Promise<UseCaseResult<{ status: string; riskTransactionId: string }>> {
    ...
    try { await assertConfiguredBindingActor(policy, actor); } catch (error) {
        return { status: 'UNAUTHORIZED', error: { code: 'JOURNEY_PERMISSION_DENIED', message: error instanceof Error ? error.message : 'This actor cannot bind this configured product.' } };
    }

    if (
        String(policy.status || '').trim().toUpperCase() === 'AWAITING_EXTERNAL_ISSUANCE'
        && input.externalIssuanceCompletion !== true
    ) {
        return {
            status: 'INVALID_STATUS',
            error: {
                code: 'EXTERNAL_ISSUANCE_COMPLETION_REQUIRED',
                message: 'This paid policy must be completed through the external insurer document workflow before it can be bound.',
            },
        };
    }
