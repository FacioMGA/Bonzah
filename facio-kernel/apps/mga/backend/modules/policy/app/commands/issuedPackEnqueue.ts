...
export type EnqueueIssuedPolicyPackArgs = {
  ...
  /** Server-only recovery mode: generate missing documents without welcome email or lifecycle promotion. */
...
export function buildIssuedPolicyPackEnvelope(args: EnqueueIssuedPolicyPackArgs) {
  ...
  const data: Prisma.InputJsonValue = {
    ...
    ...(args.documentsOnly === true ? { documentsOnly: true } : {}),
  };
 ...
 * the pack and trigger the welcome-email orchestration for normal issuance.
 * Explicit document-only recovery omits welcome email and lifecycle effects.
 */
...
export async function enqueueIssuedPolicyPackStandalone(
  ...
  return await runTenantScopedTransaction(async (tx) => {
