  ...
  router.post('/:id/cancellation/request', policyAuditLog, async (req, res) => {
    try {
      ...
      if (String(req.user?.role || '').toUpperCase() === 'CUSTOMER') {
        await assertPolicyJourneyAction(policy, 'customer', 'cancel');
      }
      ...
      if (error instanceof InsuranceConfigurationError) return sendError(res, 403, error.code, error.message);
      if (error instanceof ProgramDefinitionConfigurationError) return sendError(res, 503, error.code, error.message);
