      ...
      async listBinders() {
        ...
        return binders.map((binder) => {
          ...
          const leadCapacityProviderName =
            String(capacityProvider?.name || coverholderSecondary?.name || 'Insurer not configured');
          const authorizedClass = String(scope.authorizedClass || config.productType || 'Class not configured');
