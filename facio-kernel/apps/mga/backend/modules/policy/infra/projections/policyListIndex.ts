...
function attentionFor(policy: {
  ...
  if (status === 'AWAITING_EXTERNAL_ISSUANCE') return { bucket: 'EXTERNAL_ISSUANCE_REQUIRED', score: 92, customerActionRequired: false, uwActionRequired: true };
...
export async function rebuildPolicyListIndexRow(policyId: string) {
  ...
  // The primary row and its enrichment must share the same tenant-scoped
  // client. Creating it through the unscoped client and then enriching it
  // through the tenant-scoped client permits a cross-scope miss on an
  // otherwise valid policy during a backfill.
  await upsertPolicyListDiscoverabilityRow(tenantScopedPrisma, pid);
