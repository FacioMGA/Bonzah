...
const tenantPolicyListIndexUpsertMock = vi.fn();
  ...
  return {
    ...
    tenantScopedPrisma: {
      ...
      policyListIndex: {
        ...
        upsert: tenantPolicyListIndexUpsertMock,
...
describe('policyListIndex projection', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    tenantPolicyListIndexUpsertMock.mockResolvedValue(undefined);
  });
  ...
  it('rebuilds one index row with computed activity and attention fields', async () => {
    ...
    expect(tenantPolicyListIndexUpsertMock).toHaveBeenCalledTimes(1);
    expect(prismaMock.policyListIndex.upsert).not.toHaveBeenCalled();
  ...
  it('reconcile and backfill process policy ids and return counts', async () => {
    ...
    expect(tenantPolicyListIndexUpsertMock).toHaveBeenCalledTimes(5);
    expect(prismaMock.policyListIndex.upsert).not.toHaveBeenCalled();
