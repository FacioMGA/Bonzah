...
type CoverageSelectionParams = UnknownRecord;
type CoverageSelectionMap = Record<string, boolean>;
...
function getCoverageAllowedCodes(cfg: ProgramMbeProductConfigV1): string[] {
...
function hasAnyKeys<T>(value: Record<string, T>): boolean {
...
export function resolveEffectiveCoverageContract(args: {
  ...
  // Callers must pass the parsed coverage component from the published,
  // binder-authorised programme definition. Normalising here would recreate
  // legacy product defaults after publication and make a setting change depend
  // on code rather than the immutable definition version.
  const cfg = args.cfg;
  ...
  const resolvedCoverageSet = resolveCoverageV1({
    ...
    quoteData: args.quoteData,
    cfg,
    selectedOptions: hasAnyKeys(normalizedStored.selected) ? normalizedStored.selected : undefined,
