...
export type PaymentGatewayStatus = 'PENDING' | 'INITIATED' | 'REDIRECTED' | 'AUTHORIZED' | 'CAPTURED' | 'PAID' | 'FAILED' | 'CANCELLED';
...
const POLICY_TRANSITIONS: Record<PolicyLifecycleStatus, PolicyLifecycleStatus[]> = {
  ...
  QUOTED: ['DRAFT', 'REFERRAL', 'AWAITING_PAYMENT', 'BOUND', 'ISSUING', 'ISSUED', 'ACTIVE', 'DECLINED', 'INFO_REQUIRED', 'CANCELLATION_REQUESTED'],
  // A customer can edit a risk-scoped input after checkout begins. If that
  // invalidates an existing UW decision, the canonical lifecycle must return
  // the policy to referral rather than failing the re-rate transaction.
  AWAITING_PAYMENT: ['DRAFT', 'REFERRAL', 'AWAITING_EXTERNAL_ISSUANCE', 'BOUND', 'ISSUED', 'ACTIVE', 'DECLINED', 'QUOTED', 'CANCELLATION_REQUESTED'],
  AWAITING_EXTERNAL_ISSUANCE: ['BOUND', 'CANCELLATION_REQUESTED'],
...
const PAYMENT_TRANSITIONS: Record<PaymentGatewayStatus, PaymentGatewayStatus[]> = {
  // CardCorp creates a local PENDING checkout before the hosted widget has
  // redirected the customer. Its first verified terminal callback therefore
  // legitimately completes this state directly; cancellation remains
  // terminal and is never reversible.
