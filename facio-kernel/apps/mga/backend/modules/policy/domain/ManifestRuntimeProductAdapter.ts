import { registerEngineQuestionnaireContract } from '../../insuranceConfiguration/domain/sourceAdapters.js';
import type {
  BindRulesResult,
...
export class ManifestRuntimeProductAdapter extends BaseManifestProductAdapter {
  ...
  constructor(private readonly config: ManifestProductRuntimeConfig) {
    ...
    registerEngineQuestionnaireContract(config.manifest);
  }

  resolvePolicyPeriod(quoteData: unknown) { return this.config.resolvePolicyPeriod?.(quoteData) ?? null; }
  ...
  buildQuoteResponse(quoteData: unknown, context?: BuildQuoteResponseContext): Promise<QuoteResponseResult> {
    return this.config.buildQuoteResponse(quoteData, context);
  }
  ...
  calculateEndorsementPremium(
    ...
    return this.config.calculateEndorsementPremium(quoteData, appliedEndorsements, context);
  }
}
