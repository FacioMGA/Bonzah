...
import { evaluateBdxMigrationRows as evaluateBdxMigrationRowsInTenant } from '../service.js';
...
const { buildQuoteResponseForProduct, resolvePublishedProgramQuoteContext } = vi.hoisted(() => ({
  buildQuoteResponseForProduct: vi.fn(),
  resolvePublishedProgramQuoteContext: vi.fn(),
}));
...
const BDX_PROGRAM = { id: 'prog-1' };
const BDX_REQUEST = {
  sourceFilePath: 'ignored.xlsx',
  dryRun: true,
  productLine: 'motor' as const,
  binderProductAuthorityId: 'authority-motor',
};
const cyTenant = getTenantFixtures().find((tenant) => tenant.countryCode === 'CY')!;
const evaluateBdxMigrationRows = (args: Parameters<typeof evaluateBdxMigrationRowsInTenant>[0]) =>
  runWithOperatingTenant(cyTenant, () => evaluateBdxMigrationRowsInTenant(args));

beforeEach(() => {
  resolvePublishedProgramQuoteContext.mockResolvedValue({
    programDefinition: {
      id: 'definition-motor',
      programId: BDX_PROGRAM.id,
      version: 1,
      pricingMode: 'AUTOMATED',
      binderProductAuthorityId: BDX_REQUEST.binderProductAuthorityId,
      underwriting: {},
      coverage: {},
      questionnaire: {},
      workflow: {},
      channels: {},
      documents: {},
      ratingModel: { id: 'rating-motor', programId: BDX_PROGRAM.id, version: 1, tables: {} },
    },
    resolvedCoverageSet: { applied: [] },
  });
  buildQuoteResponseForProduct.mockResolvedValue({
    quoteResponse: {
      status: 'quoted',
      costDetails: { subtotalNetPremium: 600, totalPremium: 600, totalPayable: 600, mifSurcharge: 0, stampDuty: 0, policyFee: 0 },
      primaryOption: {
        annualPremium: 600,
        costDetails: { subtotalNetPremium: 600, totalPremium: 600, totalPayable: 600, mifSurcharge: 0, stampDuty: 0, policyFee: 0 },
      },
...
describe('bdx import evaluation', () => {
  ...
  it('blocks an issued-policy import when its source row has no NIF', async () => {
    const row = {
      Id: 'missing-nif-1',
      Entry: 'NB',
      Insured: 'No Nif',
    ...
    expect(evaluations[0]?.result).toBe('FAIL');
    expect(evaluations[0]?.gaps.some((g) => g.message.includes('Issue readiness gate failed for import'))).toBe(true);
    expect(resolvePublishedProgramQuoteContext).toHaveBeenCalledWith(expect.objectContaining({
      programId: BDX_PROGRAM.id,
      binderProductAuthorityId: BDX_REQUEST.binderProductAuthorityId,
    }));
  });
  ...
  it('evaluates rows deterministically via migration entrypoint', async () => {
    ...
    const request = BDX_REQUEST;
    const program = BDX_PROGRAM;
