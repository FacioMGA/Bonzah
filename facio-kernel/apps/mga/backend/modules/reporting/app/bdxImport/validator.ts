...
async function rateProductQuote(args: {
  ...
  if (!args.programId || !args.binderProductAuthorityId) {
    throw new Error('BDX rating requires a selected programme and binder-product authority.');
  }
  ...
  if (programmeContext.programDefinition.pricingMode !== 'AUTOMATED') {
    throw new Error('BDX rating requires an automated published programme definition.');
  }
  const response = await buildQuoteResponseForProduct({
    productType,
    quoteData: args.quoteData,
    programId: args.programId,
    binderProductAuthorityId: args.binderProductAuthorityId,
  ...
  return {
    ...
    applied: Array.isArray(programmeContext.resolvedCoverageSet?.applied)
      ? programmeContext.resolvedCoverageSet.applied
      : [],
  };
}
...
export async function runCalculabilityValidation(args: {
  ...
  const { dto, quoteData, programId, binderProductAuthorityId } = args;
  ...
  const quoteValidation = quoteValidatorModule.validateDraftQuote({
    ...
    step: 'bdx-import',
    // BDX rating is a quotation operation. Issuance-only contract fields
    // (such as Motor NIF) are checked by the canonical issue-readiness gate
    // below, after the deterministic calculation has completed.
    mode: 'quote',
    productType,
  });
  ...
  if (migrationCompliance.appliedUwAdjustment) {
    try {
      finalRating = await rateProductQuote({
        dto,
  ...
  const applied = finalRating.applied;
