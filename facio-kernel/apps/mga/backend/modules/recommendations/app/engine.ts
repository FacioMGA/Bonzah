...
function parseRequestedExcess(quoteData: QuoteDataLike, quoteResponse: QuoteResponse | null): number {
  const raw = quoteData.requiredExcess;
  const n = parseInt(String(raw || '').replace(/[^0-9]/g, '') || '0', 10);
  if (Number.isFinite(n) && n > 0) return n;
  const rated = Number(quoteResponse?.primaryOption?.voluntaryExcess);
  return Number.isFinite(rated) && rated > 0 ? rated : 0;
}
...
export class RecommendationsEngine {
  static async recommendForWorkspace(args: {
    ...
    const quoteData = args.quoteData;
    ...
    const banditSeed = String(args.quoteResponse?.reference || '') || String(parseRequestedExcess(quoteData, args.quoteResponse));
    ...
    const requestedExcess = parseRequestedExcess(quoteData, args.quoteResponse);
