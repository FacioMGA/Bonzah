...
describe('jurisdiction product configuration', () => {
  it('rejects Greece Motor for tenant, program and BDX source resolution', () => {
    for (const input of [
      { tenant: { countryCode: 'GR' } },
      { program: { metadata: { countryCode: 'GR' } } },
      { source: { countryCode: 'GR' } },
    ]) {
      expect(() => resolveJurisdictionProductConfig({ productCode: 'MOTOR', ...input })).toThrow(/MOTOR\/GR/);
    }
  });
  ...
  it('resolves PT Motor as a product/binder/country-specific config', () => {
    const config = resolveJurisdictionProductConfig({
      productCode: 'MOTOR',
      program: { id: 'program-pt-motor', productType: 'MOTOR' },
      source: { countryCode: 'PT' },
  ...
  it('resolves Portugal Open Market only as a manual referral configuration', () => {
    const config = resolveJurisdictionProductConfig({ productCode: 'OPEN_MARKET', source: { countryCode: 'PT' } });

    expect(config.countryCode).toBe('PT');
    expect(config.productCode).toBe('OPEN_MARKET');
    expect(config.taxRegime.profileCode).toBe('PT_OPEN_MARKET_MANUAL_2026');
    expect(config.bdxConfig.importDefaults.migrationSourceCode).toBe('BDX_OPEN_MARKET_PORTUGAL');
  });

  it('resolves US Rental as synthetic demo authority with no configured tax', () => {
    const config = resolveJurisdictionProductConfig({ productCode: 'RENTAL', tenant: { countryCode: 'US', country: 'United States' } });
    expect(config.countryCode).toBe('US');
    expect(config.productCode).toBe('RENTAL');
    expect(config.taxRegime.profileCode).toBe('US_RENTAL_DEMO_NO_TAX');
    expect(config.regulatoryConfig).toMatchObject({ executionMode: 'DEMO', taxCalculation: 'NOT_CONFIGURED', liveInsuranceAuthority: false });
  });
});
