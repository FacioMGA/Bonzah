 ...
 * welcome email, lifecycle promotion. Server-authored documentsOnly recovery
 * stops after successful pack validation and preserves the policy lifecycle.
 *
...
export async function runIssuedPackJob(rawJobData: unknown): Promise<unknown> {
    ...
    return runWithPolicyOperatingTenant(policyId, async () => {
        ...
        const requiredIssuedDocTypes = documentConfiguration.requiredIssuedDocTypes;
        const documentSources = documentConfiguration.documentSources;
        ...
        try {
            r = await runTenantScopedTransaction(async (tx) => {
                // Recovery and the normal issuance path can enqueue the same
                // policy concurrently. Serialise the canonical issued-pack
                // write so a second job completes the active pack instead of
                // creating another document version.
                const lockKey = `issued-pack:${policyId}:${riskTransactionId || 'none'}`;
        ...
        const missing = missingIssuedDocTypes(r, requiredIssuedDocTypes);
        if (missing.length > 0) {
        ...
        if (data.documentsOnly === true) return r;
