...
import { fixtureProgrammeDefinition, fixtureRatingModelTables } from '../../../products/programDefinitionFixtures.js';
...
describe.runIf(RUN_INTEGRATION)('Binding integrity (integration)', () => {
  ...
  let binderId: string;
  let binderProductAuthorityId: string;
  ...
  // The integration database is retained when a CI job is cancelled. A UUID
  // keeps unique-column fixtures independent of the runner's clock/randomness.
  const rnd = (prefix: string) => `${prefix}-${randomUUID()}`;
  ...
  beforeAll(async () => {
    ...
    const authority = await prisma.binderProductAuthority.create({
      data: {
        operatingTenantId: CY_TENANT.id,
        binderId,
        productCode: 'MOTOR',
    ...
    const ratingModel = await prisma.programRatingModel.create({
      data: {
        operatingTenantId: CY_TENANT.id,
        programId,
        version: 1,
        status: 'PUBLISHED',
        name: 'Binding integrity Motor fixture',
        stages: MOTOR_RATING_PIPELINE.map((operator) => ({ id: operator, operator })),
        // Test-only import builds the complete persisted programme model.
        // The HTTP journey must resolve these tables through the mapped
        // authority; the Motor rater never reads a runtime asset.
        tables: fixtureRatingModelTables('MOTOR')!,
        source: 'integration-test',
      },
      select: { id: true },
    });
    const definitionFixture = fixtureProgrammeDefinition('MOTOR');
    const definition = await prisma.programDefinitionVersion.create({
      data: {
        operatingTenantId: CY_TENANT.id,
        programId,
        version: 1,
        ...
        questionnaire: definitionFixture.questionnaire,
        workflow: { approvalRules: [], billing: {}, referralOnly: false, externalIssuance: { mode: 'NONE' } },
        channels: definitionFixture.channels,
        documents: definitionFixture.documents,
        source: 'integration-test',
      },
      select: { id: true },
    });
    await prisma.binderProductAuthorityProgramDefinition.create({
      data: {
        operatingTenantId: CY_TENANT.id,
    ...
    await prisma.binderProductAuthorityRatingModel.create({
      data: {
        binderProductAuthorityId,
        programRatingModelId: ratingModel.id,
      },
  ...
  afterAll(async () => {
    ...
    await prisma.binderProductAuthorityProgramDefinition.deleteMany({ where: { binderProductAuthorityId } }).catch(() => undefined);
    await prisma.binderProductAuthorityRatingModel.deleteMany({ where: { binderProductAuthorityId } }).catch(() => undefined);
    await prisma.programDefinitionVersion.deleteMany({ where: { programId } }).catch(() => undefined);
    await prisma.programRatingModel.deleteMany({ where: { programId } }).catch(() => undefined);
