...
import { insuranceConfigurationRouter } from '../../modules/insuranceConfiguration/index.js';
import { createPlatformUserDirectory } from './platformUsers.js';


export function createBoApiRouter() {
  const router = Router();
  router.use(requireSurfaceAccess(['bo']));
  if (process.env.KERNEL_PLATFORM_MODE === 'true') router.use(createPlatformUserDirectory());
  router.use('/insurance-configuration', requireBO, insuranceConfigurationRouter);
