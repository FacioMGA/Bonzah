...
import { contactEmailForCountry, contactPhoneForCountry, getTenantConfig, getTenantRuntimeSettings } from '../../../platform/tenant/tenantConfig.js';
...
function buildLegalContacts(countryOfResidence: string): Record<string, unknown> {
  if (process.env.KERNEL_PLATFORM_MODE === 'true') {
    const identity = getTenantRuntimeSettings(getTenantConfig());
    return {
      governingLaw: 'As stated in the selected policy wording', jurisdiction: 'As stated in the selected policy wording',
      claimProvider: identity.branding.legalName, claimEmail: identity.contact.email,
      claimTelephone: identity.contact.phone, claimAddress: identity.branding.addressLines.join(', '),
      complaintsProcedureParagraphs: identity.branding.legalLines, serviceOfSuitRecipients: [],
    };
  }
...
export function buildTravelDocViewModel(ctx: DocPackContext): UnknownRecord {
  ...
  return {
    ...
    policyWordingReference: process.env.KERNEL_PLATFORM_MODE === 'true' ? 'No insurer policy wording selected' : DEFAULT_POLICY_WORDING_REFERENCE,
    sanctionsClause: {
    ...
    coverholder: {
      legalName: tenantIdentity.branding.legalName,
      addressOneLine: tenantIdentity.branding.addressLines.join(', '),
