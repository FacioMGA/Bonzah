 ...
 */
export function resolveTravelAdminFeeFromBands(netPremium: number, feeBands: TravelFeeBands): number {
  const safeNet = Math.max(0, Number(netPremium) || 0);
  for (const band of feeBands.bands) {
...
/** File loader remains a seed/test helper, never a runtime rating source. */
export function resolveTravelAdminFee(netPremium: number): number {
  return resolveTravelAdminFeeFromBands(netPremium, loadTravelFeeBands());
}
