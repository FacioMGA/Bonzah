...
/**
 * BO Premium tab writes motor-shaped `uwAdjustments` on quoteData. Home
 * pricing consumes `risk.discretionaryDiscount` (0–0.20 ratio). Map pricing
 * discount lines into that field so recalculation applies the adjustment.
 */
export function resolveHomeDiscretionaryDiscount(data: unknown): number {
  const qd = asRecord(data);
  const risk = asRecord(qd.risk);
  const fromRisk = Number(risk.discretionaryDiscount || 0);
  const rawAdjustments = qd.uwAdjustments;
  const legacyAdjustment = asRecord(qd.uwAdjustment);
  const adjustments = Array.isArray(rawAdjustments)
    ? rawAdjustments
    : (legacyAdjustment.type || legacyAdjustment.value ? [legacyAdjustment] : []);

  let discountPctTotal = 0;
  for (const adjRaw of adjustments) {
    const adj = asRecord(adjRaw);
    if (String(adj.lineType || 'pricing').toLowerCase() === 'schedule_note') continue;
    if (String(adj.type || '').toLowerCase() !== 'discount') continue;
    if (String(adj.mode || '').toLowerCase() !== 'pct') continue;
    const value = Number(adj.value || 0);
    if (Number.isFinite(value) && value > 0) discountPctTotal += value;
  }

  if (discountPctTotal > 0) {
    return Math.max(0, Math.min(0.20, discountPctTotal / 100));
  }

  return Math.max(0, Math.min(0.20, fromRisk));
}
...
export function toHomePricingQuoteData(data: unknown, operatingCountryCode: string): HomeQuoteData {
  ...
  return enforceHomeCoverEligibility({
    ...
    discretionaryDiscount: resolveHomeDiscretionaryDiscount(qd),
  });
}
