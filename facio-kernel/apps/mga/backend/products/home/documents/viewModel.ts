...
export function buildHomeDocViewModel(ctx: DocPackContext): UnknownRecord {
  ...
  const tenantIdentity = getTenantRuntimeSettings(tenantConfig);
  ...
  const policyWordingReference = ctx.documentSources?.some((source) => source.documentType === 'HOME_POLICY_WORDING_PDF')
    ? resolveHomePolicyWording(tenantConfig.countryCode, resolveHomeWordingDomicile(qd)).reference
    : 'No insurer policy wording selected';

  const currency = String(snapshotResp.currency || tenantConfig.currency).toUpperCase();
  ...
  const ratedAssistanceFee = typeof breakdown.europAssistanceFee === 'number' ? breakdown.europAssistanceFee : undefined;
  const europAssistanceSelected =
    (ratedAssistanceFee !== undefined ? ratedAssistanceFee > 0 : undefined) ??
  ...
  const assistanceFee = Number(breakdown.europAssistanceFee ?? policyResp.europAssistanceFee ?? 0);
  const grossPremium = Number(breakdown.grossPremium ?? policyResp.annualPremium ?? netPremium + iptAmount + adminFee + assistanceFee);
  ...
  const endorsements = [
    ...
    },
    {
      ...
      body:
        'Please review the insurer policy wording selected for this policy, where attached, for the applicable insurance terms, governing law and claims procedure. Contact the operating MGA shown on this schedule if that wording is missing or unclear.',
    },
    {
      code: 'AB14',
  ...
  return {
    ...
    premium: {
      ...
      assistanceFee: num2(assistanceFee),
    ...
    premiumSummary: {
      ...
      assistanceFeeDisplay: assistanceFee !== 0 ? moneyDisplayFixed(assistanceFee) : '',
    ...
    signatory: {
      brokerLegalName: tenantIdentity.branding.legalName,
      brokerAddressOneLine: tenantIdentity.branding.addressLines.join(', '),
      authorisedSignerName: '',
      authorisedSignerTitle: `Issued by ${tenantIdentity.branding.legalName}`,
    },
    legalContacts: {
      coverholderName: tenantIdentity.branding.legalName,
      coverholderAddress: tenantIdentity.branding.addressLines.join(', '),
      serviceOfSuit: 'As stated in the selected policy wording',
      claimNotification: tenantIdentity.contact.email,
      complaintHandling: tenantIdentity.contact.email,
      governingLaw: 'As stated in the selected insurer policy wording, where attached',
      jurisdiction: 'As stated in the selected insurer policy wording, where attached',
    },
  };
}
