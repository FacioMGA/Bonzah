...
describe('resolveHomeIpid — territory-aware IPID (ADR-0048)', () => {
  it('uses the approved combined Cyprus/Greece IPID for GR', () => {
    expect(resolveHomeIpid('GR').filename).toBe('Home_IPID_Beazley_Cyprus_Greece_2023.pdf');
  });
  ...
  it('shares one exact approved asset for Cyprus and Greece', () => {
    expect(resolveHomeIpid('CY')).toBe(resolveHomeIpid('GR'));
    expect(sha256(resolveHomeIpid('GR').staticPdfPath)).toBe(
      '80268fd8f45308588a76038f0e8245d3ab81445f458e6180793a69c2a5bd47df',
    );
  });
