...
const HomeUwConfigSchema = z.object({
  allowedPropertyTypes: z.array(z.string().trim().min(1)).min(1),
  allowedRiskCountries: z.array(z.string().trim().min(1)).min(1),
  maxBuildingsSumInsured: z.number().finite().nonnegative().optional(),
  maxContentsSumInsured: z.number().finite().nonnegative().optional(),
  maxAllRisksUnspecifiedSumInsured: z.number().finite().nonnegative().optional(),
  maxSolarPanelsSumInsured: z.number().finite().nonnegative().optional(),
  declineClaimsCountOver: z.number().finite().nonnegative(),
  referClaimsCountAtLeast: z.number().finite().nonnegative(),
  combustibleConstructionRefer: z.boolean(),
  greekPostcodeDecline: z.array(z.string().trim().min(1)),
}).strict();

export function parseHomeUwConfig(value: unknown): HomeUwConfig {
  const parsed = HomeUwConfigSchema.safeParse(value);
  if (!parsed.success) {
    throw new Error(`Home underwriting configuration is incomplete or invalid: ${parsed.error.issues.map((issue) => issue.path.join('.') || issue.message).join(', ')}`);
  }
  return parsed.data;
}
