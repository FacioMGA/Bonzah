...
import { calculateAutoInsuranceQuoteResponse as calculateMotorQuoteResponse } from '../../pricing/autoInsuranceCalculator.js';
...
const calculateAutoInsuranceQuoteResponse = (...[quoteData, overrideExcess, opts, appliedEndorsements = []]: Parameters<typeof calculateMotorQuoteResponse>) =>
  runWithOperatingTenant(cyTenant, () => calculateMotorQuoteResponse(quoteData, overrideExcess, opts, appliedEndorsements, motorModel));
...
describe('motorDocs templates (smoke)', () => {
  ...
  it(
    ...
    async () => {
      ...
      const quoteDataForDocs: QuoteData = {
        ...TEST_QUOTE_DATA,
        cabrio: 'Yes',
      ...
      const quoteResponse = calculateAutoInsuranceQuoteResponse(quoteDataForDocs, undefined, {
        ...
        currency: 'EUR',
        uwDecision: evaluateMotorUwFixture(quoteDataForDocs),
      });
      ...
      const brand = {
        ...
        greenCardAuthority: 'Cyprus Motor Insurers Bureau',
        ...
        uwSignatureAsset: 'ps-signature.png',
      };
      ...
      const vm = runWithOperatingTenant(cyTenant, () => buildMotorDocViewModel({
        policy,
        snap,
        activeEndorsements: [],
