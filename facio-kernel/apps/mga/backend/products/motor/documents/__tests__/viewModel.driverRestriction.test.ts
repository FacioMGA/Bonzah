...
describe('buildMotorDocViewModel — driverRestriction wording', () => {
  it('renders EV manufacturer power in kW, never a synthetic cc value', () => {
    const vm = buildVm({
      driverRestriction: 'POLICYHOLDER_ONLY',
      quoteData: { fuelType: 'Electric', engineSize: 0, electricPowerKw: 160 },
    });
    const vehicles = vm.vehicles as Array<{ powertrain_display?: string; engine_cc?: number }>;
    expect(vehicles[0]?.powertrain_display).toBe('160 kW');
    expect(vehicles[0]?.engine_cc).toBe(0);
  });
