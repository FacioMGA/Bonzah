...
import { resolvePolicyProgrammeChannelPermissions } from '../../../modules/programs/app/policyProgrammeChannelPermissions.js';
...
export async function createSessionHandler(req: Request, res: Response) {
  try {
    ...
    const data = await createPublicAutoSession({
      ...
      origin,
      vehicleType,
      allowChannelBypass: isBackOfficeRequest(req),
    });
...
export async function patchSessionHandler(req: Request, res: Response) {
  try {
    ...
    if (!enforcePublicToken(request, resolved.mode, res)) return;
    try {
      const channel = await resolvePolicyProgrammeChannelPermissions(resolved.policy);
      if (!channel.permissions.quote && !isBackOfficeRequest(req)) {
        throw new PublicApiError({
          httpStatus: 403,
          code: 'PRODUCT_CHANNEL_DISABLED',
          message: 'Online quoting is not available for this programme right now. Please request a callback.',
        });
      }
    } catch (error) {
      if (error instanceof PublicApiError) throw error;
      if (error instanceof ProgramDefinitionConfigurationError) {
        throw new PublicApiError({ httpStatus: 503, code: error.code, message: error.message });
      }
      throw error;
    }
