...
import { normalizeProgramMbeProductConfig } from '../../../../modules/mbe/domain/programProduct.js';
...
describe('rateQuote edge cases', () => {
  ...
  it('keeps a current manual approval when a customer rerates the same referred motor risk', async () => {
    const programMetadata = { abbeygateMotorUwConfig: {} };
    const binderProductAuthority = {
      id: 'authority-1', productCode: 'MOTOR', status: 'ACTIVE', classOfBusiness: 'MOTOR', riskCode: 'MC',
      territorialScope: ['CY'], maxPremiumAnnual: '10000', maxPolicyPeriodDays: 365,
      maxAdvanceInceptionDays: 60, authorityClasses: ['COMPREHENSIVE'], effectiveFrom: null, effectiveTo: null,
    };
    const quoteData = { ...baseQuoteData, requiredExcess: '€250', vehicleValue: 50_000, marketValue: 20_000 };
    vi.mocked(tenantScopedPrisma.policy.findUnique).mockResolvedValue({
      id: 'pol-1', policyNumber: 'ABQ1', productType: 'MOTOR', status: 'REFERRAL', isLocked: false,
      programId: 'program-1', binderId: 'binder-1',
    });
    vi.mocked(tenantScopedPrisma.program.findUnique).mockResolvedValue({ id: 'program-1', metadata: programMetadata });
    vi.mocked(tenantScopedPrisma.binderProductAuthority.findUnique).mockResolvedValue(binderProductAuthority);

    const firstRating = await rateQuoteCY({ policyId: 'pol-1', quoteData, overrideExcess: 1 });
    expect(firstRating.status).toBe('referral');
    const firstPersisted = parseRecord(
      vi.mocked(tenantScopedPrisma.policyStateCurrent.upsert).mock.calls.at(-1)?.[0]?.update?.snapshot,
    );
    const customerCompletionPaths = ProductRegistry.getInstance().getAdapter('MOTOR')!.getManualUwApprovalCustomerCompletionPaths();
    const approvedRiskHash = computeManualUwApprovedRiskHash({
      quoteData: firstPersisted.quoteData,
      coverageSelection: firstPersisted.coverageSelection,
      authority: buildManualUwApprovalAuthority({
        binderId: 'binder-1', programId: 'program-1', programMetadata, binderProductAuthority,
        normalizedMbeProductConfig: normalizeProgramMbeProductConfig(undefined),
      }),
      underwritingDecision: firstPersisted.underwritingAnalysis,
      customerCompletionPaths,
    });
    vi.mocked(tenantScopedPrisma.policyStateCurrent.findUnique).mockResolvedValue({
      snapshot: {
        ...firstPersisted,
        uw: { manualApproval: { approvedAt: '2026-09-04T00:00:00.000Z', approvedRiskHash, customerCompletionPaths } },
        pricing: {
          ...parseRecord(firstPersisted.pricing),
          boOverrideApproval: { requestedOverrideExcess: 1 },
        },
      },
    });

    const result = await rateQuoteCY({ policyId: 'pol-1', quoteData });

    expect(result.status).toBe('QUOTED');
    expect(tenantScopedPrisma.policy.update).toHaveBeenLastCalledWith(expect.objectContaining({
      data: expect.objectContaining({ status: 'QUOTED' }),
    }));
    const persisted = vi.mocked(tenantScopedPrisma.policyStateCurrent.upsert).mock.calls.at(-1)?.[0];
    expect(persisted?.update?.snapshot?.uw?.manualApproval).toMatchObject({ approvedRiskHash });
  });
