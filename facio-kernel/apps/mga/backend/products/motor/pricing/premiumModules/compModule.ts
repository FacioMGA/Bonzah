...
export function calculateCompPremium(inputs: CompInputs): CompResult {
    ...
    if (isMotorbike) {
        ...
        const riderAgeFactor = motorcycleRiderAgeFactorFromWorkbook(age, matrix.motorcycle.riderAgeFactors);
        const ncdFactor = motorcycleNcdFactorFromWorkbook(String(normalizedQuoteData.ncb || ''), matrix.motorcycle.ncdFactors);
        ...
        , matrix.motorcycle.countryFactors);
        const fixedFee = motorcycleFixedFeeFromWorkbook(
            String(normalizedQuoteData.countryOfRegistration || normalizedQuoteData.vehicleLocation || '')
        , matrix.motorcycle.fixedFees);
        ...
        if (isTpo) {
            tplOverride = fromDecimal(roundCurrency(toDecimal(workbookCompBeforeTpo).mul(matrix.tpl.tpoFactor).plus(fixedFee)));
            compRaw = 0;
        } else {
            tplOverride = 0;
