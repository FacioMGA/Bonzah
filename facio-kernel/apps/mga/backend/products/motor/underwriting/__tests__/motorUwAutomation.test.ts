...
import { evaluateMotorUwFixture as evaluateMotorUwAutomation } from './fixtureHelper.js';
...
describe('vehicle referral requirements from EV directive', () => {
  it('refers electric vehicles before quote', () => {
    const result = evaluateMotorUwAutomation(baseCarQuote({ fuelType: 'Electric', electricPowerKw: 160 }));
    expect(result.outcome).toBe('referral');
    expect(result.triggers?.some((t) => t.ruleId === 'YELLOW.EV_REFERRAL' && t.lane === 'yellow')).toBe(true);
    expect(result.triggers?.find((t) => t.ruleId === 'YELLOW.EV_REFERRAL')?.message)
      .toBe('Electric vehicle requires referral to our underwriters.');
  });
