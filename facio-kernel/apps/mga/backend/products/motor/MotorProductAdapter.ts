import { registerEngineQuestionnaireContract } from '../../modules/insuranceConfiguration/domain/sourceAdapters.js';
import type {
  IProductAdapter,
...
import { applyMotorReferralOverlays } from './underwriting/referralOverlays.js';
...
export class MotorProductAdapter implements IProductAdapter {
  ...
  constructor() { registerEngineQuestionnaireContract(motorManifest); }
  ...
  async buildQuoteResponse(quoteData: unknown, context?: import('../../modules/policy/domain/productContracts.js').BuildQuoteResponseContext): Promise<QuoteResponseResult> {
    const result = await motorProductRuntimeDefinition.buildQuoteResponse(quoteData, context);
    return applyMotorReferralOverlays({ quoteData, context, result });
  }

  validateBindRules(quoteData: unknown, binderConfig: unknown): BindRulesResult {
    try {
      resolveJurisdictionProductConfig({ productCode: this.productType, tenant: getTenantConfig() });
    } catch (error) {
      if (!(error instanceof ProductConfigurationError)) throw error;
      return { valid: false, errors: [{ field: 'productType', message: error.message }] };
    }
  ...
  async validateForIssuance(quoteData: unknown): Promise<IssuanceValidationResult> {
    try {
      ...
      if (!(error instanceof ProductConfigurationError)) throw error;
      return { valid: false, schemaIssues: [{ slug: 'productType', message: error.message }], missingSlugs: [], missingForQuotePack: [], missingForIssuedPack: [], conditionalRequirements: [] };
    }
  ...
  async calculateEndorsementPremium(
    ...
    return motorProductRuntimeDefinition.calculateEndorsementPremium(quoteData, appliedEndorsements, context);
  }
}
