 ...
 * inline.
 *
 * GHS extension adds doctor visits & medications at no premium impact —
 * the bonus rides on whether `ghs.isBeneficiary === true` and is
 * reflected in the document view-model, not the calculator total.
 * Standard outpatient cover is always included in base Section A.
 *
 ...
 */
export function calculateHealthPremium(data: HealthQuoteData, rates: BritHealthRates): {
  ...
  for (const row of insuredsRaw) {
    ...
    const bandRow = rates.ageBands.find((row) => row.band === band) ?? null;
    if (!bandRow) {
      return {
        premium: {
          premium: 0, basis: 'HYBRID',
  ...
  if (ghsExtensionApplied) {
    steps.push({
      ...
      name: 'GESY extension — doctor visits & medications (no premium impact)',
      kind: 'fee',
      amount: 0,
      inputs: { doctorVisit: rates.ghsExtension.doctorVisit, medications: rates.ghsExtension.medications },
    });
  }
