...
`Azure Images` applies a bounded 25-minute limit to each API/worker image job. The worker image carries document-generation dependencies and may legitimately exceed the API build duration; a timeout remains a failed image release and must not be promoted.
