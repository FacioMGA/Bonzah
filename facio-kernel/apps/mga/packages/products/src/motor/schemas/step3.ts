...
export const Step3Schema = z
  .object({
    ...
    engineSize: z.number(),
    /** Manufacturer maximum combined power in kW for EVs (ADR-0102). */
    electricPowerKw: z.number().positive().nullable().optional(),
  ...
  .superRefine((data, ctx) => {
    ...
    // ADR-0102 makes manufacturer maximum combined power in kW the only
    // EV power input. There is no kWh or 1cc compatibility value.
    ...
    if (isElectric) {
      if (data.electricPowerKw === undefined || data.electricPowerKw === null || !Number.isFinite(data.electricPowerKw) || data.electricPowerKw <= 0) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['electricPowerKw'],
          message: 'Please enter the manufacturer maximum combined power (kW) for the electric vehicle',
        });
      }
    } else if (isCaravan) {
      ...
      if (data.electricPowerKw !== undefined && data.electricPowerKw !== null) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['electricPowerKw'],
          message: 'Electric motor power (kW) must only be set for electric vehicles',
        });
      }
      ...
      if (data.electricPowerKw !== undefined && data.electricPowerKw !== null) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['electricPowerKw'],
          message: 'Electric motor power (kW) is only valid for electric vehicles',
        });
      }
    }
