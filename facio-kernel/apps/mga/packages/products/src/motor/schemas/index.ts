...
const FIELD_LABELS: Record<string, string> = {
  ...
  electricPowerKw: 'Electric motor power',
...
function validateMotorStageSemantics(data: unknown): Record<string, string> {
  ...
  // ADR-0102: the public wizard and every direct bind/issue route use the
  // same EV power requirement. The value must be sourced; it is never
  // inferred from battery capacity, make/model, or an engine-size sentinel.
  const fuelType = String(obj.fuelType || '').trim();
  const electricPowerKw = Number(obj.electricPowerKw);
  if (fuelType === 'Electric') {
    if (!Number.isFinite(electricPowerKw) || electricPowerKw <= 0) {
      errors['electricPowerKw'] = 'Manufacturer maximum combined power (kW) is required for an electric vehicle';
    }
  } else if (obj.electricPowerKw !== undefined && obj.electricPowerKw !== null && obj.electricPowerKw !== '') {
    errors['electricPowerKw'] = 'Electric motor power (kW) is only valid for electric vehicles';
  }
