...
describe('Step3Schema engine size and EV power validation (ADR-0102)', () => {
  ...
  it('accepts an electric vehicle when manufacturer combined power is present', () => {
    const parsed = Step3Schema.safeParse({ ...base, fuelType: 'Electric', engineSize: 0, electricPowerKw: 160 });
    expect(parsed.success).toBe(true);
  });

  it('rejects electric vehicles without manufacturer combined power', () => {
    const parsed = Step3Schema.safeParse({ ...base, fuelType: 'Electric', engineSize: 0 });
    expect(parsed.success).toBe(false);
    if (!parsed.success) {
      expect(parsed.error.issues.some((i) => i.path.join('.') === 'electricPowerKw')).toBe(true);
    }
  });
