...
export const MOTOR_WHERE_DID_YOU_HEAR_OPTIONS: SelectOption[] = [
  ...
  { value: 'Algarve Classified', label: 'Algarve Classified' },
  { value: 'Algarve Daily News', label: 'Algarve Daily News' },
  { value: 'Bing', label: 'Bing' },
  { value: 'British Chamber of Commerce', label: 'British Chamber of Commerce' },
...
const vehicleTypeReviewLabel: Partial<Record<string, string>> = {
  ...
  Motorcaravan: 'Motorhome / Motor Caravan (manual review)',
...
export const motorManifest: ProductManifest = {
  ...
  insuredObject: {
    ...
    fields: [
      ...
      { path: 'electricPowerKw', label: 'Electric motor power (kW)', type: 'number' },
  ...
  summaryFields: {
    ...
    subtitlePaths: ['fuelType', 'engineSize', 'electricPowerKw'],
  ...
  listColumns: {
    insured: {
      ...
      buildSecondary: (data) => {
        ...
        const electricPower = String(data.electricPowerKw || '').replace(/\D/g, '').trim();
        const powertrain = fuel === 'Electric' ? (electricPower ? `${electricPower}kW` : '') : (engine ? `${engine}cc` : '');
        return [fuel, powertrain].filter(Boolean).join(' · ') || null;
      },
    },
    coverage: {
  ...
  riskModelHints: {
    requiredForUw: [
      ...
      { path: 'electricPowerKw', label: 'Electric motor power (kW)' },
    ...
    ratingInputs: [
      ...
      { path: 'electricPowerKw', label: 'Electric motor power (kW)', format: 'text' },
