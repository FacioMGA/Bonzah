...
describe('motor wizard ↔ rate-API contract (ABY-106)', () => {
  ...
  it('allows a quote without NIF but requires it when the customer proceeds to issuance', () => {
    const quoteErrors = validateForContext({
      productCode: 'MOTOR',
      stage: { kind: 'stage', id: 'quote' },
      actor: 'customer',
      data: FULLY_FILLED_MOTOR_FIXTURE,
    ...
    expect(quoteErrors['proposer.nif']).toBeUndefined();
    expect(issuanceErrors['proposer.nif']).toBeTruthy();
  });
