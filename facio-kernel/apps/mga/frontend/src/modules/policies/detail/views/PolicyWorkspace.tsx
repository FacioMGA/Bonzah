...
export function PolicyWorkspace(props: PolicyWorkspaceProps) {
  ...
  if (asRecord(selectedPortfolio).loadError) return <section className="ui-card ui-card-pad m-6 max-w-xl space-y-4"><h1 className="text-2xl font-bold">Policy unavailable</h1><p role="alert">{String(asRecord(selectedPortfolio).loadError)}</p><p className="text-sm text-slate-600">The selected workspace has not returned this policy. Choose an authorized policy from its list.</p><Button type="button" variant="secondary" disabled={value.loading} onClick={()=>void value.loadPolicyDetails()}>Retry policy lookup</Button><Button type="button" onClick={()=>navigate('/policies')}>Back to policies</Button></section>;
  if (!asRecord(selectedPortfolio).productType && selectedPortfolio.id !== 'new' && !selectedPortfolio.isNew) return <p role="status" className="ui-page">Loading the selected policy…</p>;
              ...
              */}
              ...
              {activeTab === 'Documents' && (
                <PolicyDocuments
                  ...
                  canGenerateDocuments={hasPermission(user, 'documents.generate')}
                  onRefreshDocuments={value.loadPolicyDetails}
                  onExternalIssuanceCompleted={async () => {
                    await value.loadPolicyDetails();
                    await value.loadPolicies();
                  }}
                />
              )}
            </div>
