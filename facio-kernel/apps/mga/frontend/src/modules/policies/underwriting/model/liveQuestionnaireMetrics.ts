  ...
  return args.questionnaireStructure.reduce<LiveQuestionnaireMetrics>(
    (acc, section) => {
      for (const question of section.questions) {
        ...
        if (!fieldKey || question.type === 'paragraph') continue;
        ...
        const visible = isQuestionVisibleForUnderwriting({
          ...
          sourceScopeError: question.sourceScopeError,
        ...
        const stages = question.requiredAtStages ?? keyContract.requiredAtByActor?.underwriter ?? keyContract.requiredAtStages;
        const required = stages ? stages.includes(args.underwritingStage)
          : isFieldEffectivelyRequired(args.productType, fieldKey, { actor: 'underwriter', stage: args.underwritingStage }, currentAnswers);
