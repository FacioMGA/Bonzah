...
export function Underwriting(props: UnderwritingProps) {
  ...
  const programmeDefinition = React.useMemo(() => {
    const definition = asRecord(asRecord(selectedPortfolio).programmeDefinition);
    return String(definition.programId || '') === String(selectedProgramId || '')
      ? definition
      : null;
  }, [selectedPortfolio, selectedProgramId]);
  const programmeDefinitionError = asRecord(asRecord(selectedPortfolio).programmeDefinitionError);
  ...
  const questionnaireStructure = React.useMemo(
    () => resolveProgramQuestionnaire(programmeDefinition, { actor: 'underwriter', stage: underwritingStage }),
    [programmeDefinition, underwritingStage]
  );
  ...
  const liveQuestionnaireMetrics = React.useMemo(() => {
    return computeLiveQuestionnaireMetrics({
      ...
      productType: resolvedProductType,
      underwritingStage,
      binderProductAuthorityId: String(asRecord(asRecord(selectedPortfolio).programmeDefinition).binderProductAuthorityId || ''),
    ...
    underwritingStage,
    selectedPortfolio?.programmeDefinition,
  ]);
  return (
    <>
      <div className="space-y-8 max-w-full relative pb-24">
        ...
        <div className="space-y-8">
              ...
              {!selectedProgramId ? (
              ...
              ) : String(programmeDefinitionError.message || '').trim() ? (
                <div className="rounded-2xl border border-amber-200 bg-amber-50/60 p-4 text-sm font-semibold text-amber-800">
                  {String(programmeDefinitionError.message)}
                </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4 text-sm font-semibold text-slate-700">
                  The selected program has no published questionnaire configuration for this binder authority.
                </div>
              ) : null}
