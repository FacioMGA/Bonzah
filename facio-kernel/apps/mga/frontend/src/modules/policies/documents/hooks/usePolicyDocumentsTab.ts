...
export function usePolicyDocumentsTab(args: UsePolicyDocumentsTabArgs) {
  ...
  const target = useRef(selectedPolicyId);
  target.current = selectedPolicyId;
  ...
  const refreshPolicyDocuments = useCallback(async (policyId: string) => {
    ...
    try {
      ...
      if (request !== requestEpoch.current || target.current !== policyId) return;
  ...
  useEffect(() => { setDocs([]); requestEpoch.current++; }, [selectedPolicyId]);
