import { getTenantDisplayName } from '@/src/shared/lib/tenant/runtimeProfile';
import React from 'react';
import { CheckCircle2, FileText, ShieldCheck } from 'lucide-react';
import { Button } from '@/src/shared/ui';
import { asRecord, type UnknownRecord } from '@/src/shared/lib/record';
import { PaymentStep } from '../steps/PaymentCapabilityGate';
...
export function ManualProposalQuoteView(props: ManualProposalQuoteViewProps) {
  ...
  const [showPayment, setShowPayment] = React.useState(() => {
    ...
    return (
      sp.get('step') === 'payment' || sp.has('id') || sp.has('resourcePath') || sp.has('result')
    );
  });
  ...
  const reference =
    text(quoteResponse.reference) ||
    text(quoteResponse.quoteReference) ||
    text(quoteData.reference);
        ...
        <main className="mx-auto flex min-h-screen max-w-3xl items-center px-5 py-10">
          <div className="w-full rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm">
            ...
            <p className="mx-auto mt-3 max-w-xl text-sm font-semibold leading-6 text-slate-500">
              {paid
                ? `Thank you. ${getTenantDisplayName()} is issuing your policy from the approved proposal. Your documents will follow shortly.`
                : `Your proposal has not been paid yet. Please try again or contact ${getTenantDisplayName()} if the problem continues.`}
            </p>
            {reference && (
              <div className="mt-6 text-xs font-black uppercase tracking-widest text-slate-400">
                Reference {reference}
              </div>
            )}
          </div>
      ...
      <div className="brand-flow min-h-screen overflow-x-hidden bg-white text-black font-sans">
        <header className="border-b border-slate-100 bg-white">
          <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-5">
            <div className="font-black text-brand-primary">{getTenantDisplayName()}</div>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
          <section className="space-y-6">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm font-semibold leading-6 text-slate-500">
                Hi {proposerName(quoteData)}, {getTenantDisplayName()} has prepared your proposal.
                Please review the cover, limits, excesses and premium below.
              </p>
            </div>
                    ...
                    {rows.length === 0 ? (
                      ...
                      rows.map((row, index) => (
                        <tr key={index}>
                          ...
                          <td className="px-6 py-5 text-right font-black tabular-nums text-slate-900">
                            {money(row.premium, currency)}
                          </td>
          ...
          <aside className="space-y-4 lg:sticky lg:top-6 lg:self-start">
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-2 text-3xl font-black tabular-nums text-brand-primary">
              ...
              <div className="mt-4 space-y-3 border-t border-slate-100 pt-4 text-sm">
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span className="text-right font-black text-slate-900">
                    {text(proposal.marketName) || getTenantDisplayName()}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-blue-100 bg-blue-50 p-5 text-sm font-semibold leading-6 text-slate-700">
              Cover starts only after you approve this proposal, complete payment, and{' '}
              {getTenantDisplayName()} issues confirmation.
            </div>
            ...
            <label className="flex items-start gap-3 rounded-3xl border border-slate-200 bg-white p-5 text-sm font-semibold leading-6 text-slate-600 shadow-sm">
              ...
              />
              <span>
                I accept this proposal, including the cover, limits, excesses, premium, terms notes
                and subjectivities shown above.
              </span>
            </label>
            ...
            {!canPay && (
              <div className="text-center text-xs font-semibold text-slate-400">
                Payment is available once {getTenantDisplayName()} has finalized the premium.
              </div>
            )}
          </aside>
        </div>
