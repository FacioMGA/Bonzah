...
describe('sessionAdapter — public session wire contract', () => {
  ...
  afterEach(() => {
    vi.unstubAllGlobals(); clearSelectedTenant(); localStorage.clear();
  });

  it('forwards the exact selected JWT, operating tenant and account scope for all journey operations', async () => {
    installSelectedTenant(presentationTenant());
    localStorage.setItem('active_operating_tenant_id', 'tenant-alpha'); localStorage.setItem('auth_token', 'selected-jwt');
    const adapter = createSessionAdapter({ productCode: 'home' });
    await adapter.load('retained'); await adapter.patch('retained', {}); await adapter.rate('retained'); await adapter.issue('retained');
    for (const [, init] of fetchSpy.mock.calls) expect(init.headers).toMatchObject({ Authorization: 'Bearer selected-jwt', 'X-Tenant-Slug': 'tenant-alpha', 'x-tenant-id': 'account-tenant-alpha' });
    localStorage.setItem('active_operating_tenant_id', 'tenant-beta');
    await expect(adapter.patch('retained', {})).rejects.toThrow('active workspace changed');
  ...
  it('load() GETs the token-keyed session URL with the token URL-encoded', async () => {
    ...
    expect(fetchSpy).toHaveBeenCalledWith('/api/public/travel/session/tok%20with%20space', { headers: {} });
    expect(result.ok).toBe(true);
