                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-white border border-slate-200 rounded-2xl p-5">
                        ...
                        {isEditing ? (
                            ...
                            <div className="mt-2 text-sm font-black text-slate-900">{program.name} • {program.productType || 'Product'}</div>
                        )}

                        <div className="mt-2 text-sm text-slate-600 font-semibold flex items-center gap-2">
                            Binder: <span className="font-black text-slate-800">{linkedBinders.length ? `${linkedBinders.length} Linked` : '—'}</span>
                        </div>
                    </div>

                    <div className="bg-white border border-slate-200 rounded-2xl p-5">
                        <div className="text-[10px] font-black uppercase tracking-widest text-slate-400">Runtime settings</div>
                        <div className="mt-2 text-sm font-black text-slate-900">Definition controlled</div>
                        <div className="mt-2 text-sm text-slate-600 font-semibold">Underwriting, channels and workflow are published with the mapped programme definition.</div>
                    </div>
