...
describe('Step3VehicleCover trim lookup', () => {
  ...
  it('uses the canonical date picker for policy start dates', () => {
    renderWithForm(initialQuoteData);
    const picker = screen.getByLabelText('Policy start date') as HTMLInputElement;

    expect(picker).toHaveAttribute('data-date-input', 'true');
    expect(screen.getByRole('button', { name: /open policy start date picker/i }))
      .toHaveAttribute('data-date-picker-overlay', 'true');
  });
  ...
  it.each([
    ['Van to 3.5 tons', 'Van'],
    ['Motorcaravan', 'Motorhome / Motor Caravan'],
  ...
  it.each(['Motorbike', 'Van to 3.5 tons', 'Motorcaravan'])('does not offer Cabrio for %s', (vehicleType) => {
    renderWithForm({ ...initialQuoteData, vehicleType: vehicleType as QuoteData['vehicleType'] });

    expect(screen.queryByText(/^Cabrio$/)).toBeNull();
  });

  it('offers Cabrio for passenger cars', () => {
    renderWithForm({ ...initialQuoteData, vehicleType: 'Car' });

    expect(screen.getByText(/^Cabrio$/)).toBeInTheDocument();
  });

  it('does not expose registration before a quote is accepted', () => {
    renderWithForm(initialQuoteData);

    expect(screen.queryByText(/^Registration Number$/i)).toBeNull();
    expect(screen.getByText(/VIN \(optional\)/i)).toBeInTheDocument();
  });
