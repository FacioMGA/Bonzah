...
import { travelIpidHref } from '../../travelIpidHref';
...
function PlanCard({ plan, premium, isSelected, connectedTop = false, tripTypeLabel, planType, onSelect }: PlanCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-2.5 border-b border-slate-100 bg-slate-50/50 space-y-1">
        {getPlanDocuments().map((doc) => (
          <a
            key={doc.label}
            href={doc.href === '/api/public/ipid/travel' ? travelIpidHref(planType) : doc.href}
