...
describe('TravelQuoteSidebar', () => {
  ...
  it.each(['single_trip', 'annual_multi_trip'])('links the sidebar disclosure to the selected %s cover', (planType) => {
    render(<TravelQuoteSidebar areaLabel="Europe" planType={planType} coverLevelLabel="Gold" coveringLabel="Single" startDate="01/09/2026" endDate="01/09/2027" showDocs showContact={false} />);
    expect(screen.getByRole('link', { name: 'Insurance Product Information (IPID)' })).toHaveAttribute('href', `/api/public/ipid/travel?variant=${planType}`);
  });

  it('does not link an unspecified trip type to Single-Trip disclosure', () => {
    render(<TravelQuoteSidebar areaLabel="Europe" coverLevelLabel="" coveringLabel="Single" startDate="" endDate="" showDocs showContact={false} />);
