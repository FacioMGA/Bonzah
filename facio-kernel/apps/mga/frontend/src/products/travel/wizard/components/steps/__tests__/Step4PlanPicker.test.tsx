...
function Harness(props: {
  ...
  const form = useForm({
    defaultValues: {
      ...
      trip: {
        planType: props.defaultPlanType ?? 'single_trip',
        destinations: ['europe'],
...
describe('Step4PlanPicker', () => {
  ...
  it.each(['single_trip', 'annual_multi_trip'])('links every plan card to the selected %s IPID', (planType) => {
    render(<Harness defaultPlanType={planType}><Step4PlanPicker loading={false} quoteResponse={quoteResponse} onRate={vi.fn()} /></Harness>);
    const disclosures = screen.getAllByRole('link', { name: 'Insurance Product Information (IPID)' });
    expect(disclosures.length).toBeGreaterThan(0);
    for (const link of disclosures) expect(link).toHaveAttribute('href', `/api/public/ipid/travel?variant=${planType}`);
  });

  it('does not default a missing plan type to a Single-Trip IPID link', () => {
    render(<Harness defaultPlanType=""><Step4PlanPicker loading={false} quoteResponse={quoteResponse} onRate={vi.fn()} /></Harness>);
    expect(screen.queryByRole('link', { name: 'Insurance Product Information (IPID)' })).not.toBeInTheDocument();
  });
