...
import { businessManifest, commercialManifest, healthManifest, homeManifest, motorManifest, openMarketManifest, rentalManifest, travelManifest } from '@facio/products';
import type {
...
export type FrontendProductCatalogEntry = {
  ...
  publicJourneyAvailable?: boolean;
  firstStep: string;
...
const homePresentation: WizardPresentationProfile = {
  ...
  inclusions: [
    'Review the selected buildings and contents amounts',
    'Cover remains subject to the retained policy terms',
  ],
...
const healthPresentation: WizardPresentationProfile = {
  ...
  celebration: { variant: 'subtle' },
  inclusions: [
    'Inpatient, outpatient & repatriation cover',
    'Doctor visits & medications for GESY beneficiaries',
    '14-day cooling-off period',
  ],
...
const businessPresentation: WizardPresentationProfile = {
  ...
  heroSubline: (values) => {
    ...
    return businessType ? `Manual commercial review for ${businessType}` : 'Manual commercial review by the underwriting team';
  },
...
const rentalPresentation: WizardPresentationProfile = {
  productLabel: 'rental vehicle protection',
  heroEyebrow: 'Bonzah protection',
  heroHeadline: 'Protect your rental vehicle',
  heroSubline: () => 'Flexible protection for your United States rental',
  background: { from: '#fff8fb', via: '#ffffff', to: '#f8edf3' },
  celebration: { variant: 'subtle' },
  inclusions: ['Collision damage waiver', 'Optional liability protection', 'Personal accident and effects options'],
  loadingTitle: 'Calculating your rental protection…',
  loadingArtwork: 'vehicle',
};
...
export const productCatalog: FrontendProductCatalogEntry[] = [
  ...
  {
    manifest: commercialManifest, publicEntryPath: '/quote/commercial/new', publicJourneyAvailable: false,
    firstStep: 'policy-holder', publicSessionSlug: 'commercial',
    presentation: {...businessPresentation, productLabel: 'commercial insurance', heroHeadline: 'Commercial insurance quote', heroEyebrow: 'Quote review'},
  },
  {
    manifest: motorManifest,
    availableCountryCodes: ['CY', 'PT', 'ES'],
