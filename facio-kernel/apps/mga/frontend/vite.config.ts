...
export default defineConfig(({ mode }: { mode: string }) => {
    ...
    return {
      ...
      server: {
        ...
        host: '0.0.0.0',
        proxy: {
          '/auth': { target: process.env.FACIO_API_PROXY_URL || process.env.VITE_PROXY_TARGET || 'http://localhost:3000', changeOrigin: false },
          '/api': {
            target: process.env.FACIO_API_PROXY_URL || process.env.VITE_PROXY_TARGET || 'http://localhost:3000',
            changeOrigin: false,
          },
        },
      },
