...
2. **The `delta` column is the only place a launch draft accumulates changes.** Its shape is declared in `backend/modules/configuration/domain/draftDelta.ts` and is **strictly typed** — Zod-validated on every write tool call. The shape mirrors the V1 tool catalog one-to-one:
   ...
   `DraftDelta` is the only write path from the MCP tool surface to canonical configuration. There is no other shape, no second helper, no untyped JSON patch.

3. **Publish is the only canonical write.** `backend/modules/configuration/app/publishToSandbox.ts` runs **one** Prisma transaction inside `runWithOperatingTenant(syntheticTenantConfig, ...)`. Inside, it delegates each delta family to the canonical write helpers that already own those concerns:
   - `uwOverrides` / `mbeOverrides` / `questionnaireOverrides` / `approvalRules` / `jurisdictionOverrides` → `programs` module's existing `metadata` patch service.
   - `binderAuthorityOverrides` → [`backend/modules/policy/app/binders/`](../../../backend/modules/policy/app/binders/) binder authority service.
   - `billing.commissionPercent` → `BinderFinancials.commissionRate`.
   - `billing.adminFee` → `Tenant.adminFee` (only if explicitly set; tenant-wide write requires explicit confirmation).

   The publish step writes `publishedProgramId` / `publishedBinderId` back onto the draft and flips `status` to `sandbox_published`. After publish, **no reader of canonical configuration ever reads `ProductLaunchDraft` again**. The draft becomes evidence-only.
...
5. **Sandbox target is `TenantKind.SYNTHETIC`.** V1 publish refuses to write when the target tenant's `kind !== 'SYNTHETIC'`. `publish_production` is reserved (ADR-0036 §6) and unreachable in V1 — no tool wires it.
...
- Long-lived `ProductLaunchDraft` rows whose `status === 'sandbox_published'` being mutated. Re-configuration creates a new draft.
- `publishToSandbox` writing to a `PRODUCTION` tenant.

## Consequences

- One new Prisma model, one new module-scope column shape, zero new canonical owners — the contract spine is preserved.
- Existing canonical owners (manifest, validation profile, program metadata, binder authority, jurisdiction config) remain unchanged; their guards keep working untouched.
