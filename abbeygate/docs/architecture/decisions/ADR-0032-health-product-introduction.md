...
7. **GHS extension is a conditional cover block, not a paid addon.** The MBE endorsement `HEALTH-GHS-EXTENSION` carries `option_defaults.selectedWhen: [{ path: 'ghs.isBeneficiary', equals: true }]`. The wizard, BO Coverage tab, and PDF schedule all read MBE-resolved endorsements — one source of truth.
