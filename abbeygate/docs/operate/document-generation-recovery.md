...
- **Per-policy recovery (failure already audited)** → trigger `enqueueIssuedPolicyPackStandalone({ policyId })` from a one-off task; the readiness evaluator auto-flips `failed` → `pending` → `issued` once a new doc lands with `generatedAt` newer than the audit row.
