...
- `tenants WHERE tenantSlug='abbeygate-config-mcp-sandbox' AND kind='SYNTHETIC'` MUST exist.

## Sandbox publish failed (Config MCP)
- Check `audit_actions WHERE actionName LIKE 'CONFIG.TOOL_CALLED.config.publish.publishToSandbox'`. Draft must be `simulated`; `confirmationText` must exactly match `PUBLISH <draft.name> TO SANDBOX`.

## Drift checks
- Config MCP: `configuration-overlay-bounds`, `product-launch-draft-isolation`.
...
## Rolling back a published sandbox program
- Manual: `UPDATE programs SET status='ARCHIVED' WHERE id=<id>` + same for `binders`. Keep the `product_launch_drafts` row as evidence.

## Permissions
- `configuration.{read,draft,validate,simulate}` → UNDERWRITER. `configuration.publish_sandbox` → ADMIN only. `publish_production` reserved.
