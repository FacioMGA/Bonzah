...
import { useProductChannel } from '@/src/shared/lib/productChannels';
import {
...
export function PaymentStep({
  ...
  // ADR-0046 — when online payment is switched off for this product/tenant,
  // show a referral completion instead of the checkout (admins bypass). The
  // backend also refuses the checkout, so this is the graceful UX mirror.
  const { channel: productChannel } = useProductChannel(productCode);
  ...
  // ADR-0046 — payment channel OFF (and not returning from the gateway, and no
  // checkout already created): show a referral completion. A customer who has
  // already been redirected to the gateway (hasGatewayParams) must still be able
  // to finish, so we never block that path.
  const paymentBlocked = productChannel ? !productChannel.payment : false;
  if (paymentBlocked && !hasGatewayParams && !checkout) {
    return (
      <motion.div
      ...
      >
        <div className="flex items-center gap-4 mb-8">
          <Button
            ...
            onClick={onBack}
            variant="secondary"
            className="group p-2 -ml-2 rounded-full transition-all hover:bg-gray-100"
            aria-label="Back"
          >
        ...
        <div className="ui-card ui-card-pad text-center">
          ...
          <div className="mt-4 text-lg font-bold text-gray-900">An advisor will complete your purchase</div>
          <p className="mt-2 text-sm text-gray-600">
            Online payment isn't available for this product yet. We've captured your details and an
            Abbeygate advisor will call you shortly to finalise your cover and take payment.
          </p>
          <p className="mt-1 text-xs text-gray-400">
            {summary.currency} {Number(summary.amount || 0).toFixed(2)} — indicative premium
          </p>
        </div>
      </motion.div>
    );
