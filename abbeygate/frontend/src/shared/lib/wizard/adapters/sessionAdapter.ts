...
export function createSessionAdapter({ productCode }: { productCode: string }): PublicSessionAdapter {
  ...
  return {
    async create(seed) {
      const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(seed || {}) });
    ...
    async load(token) {
      const res = await fetch(`${url}/${encodeURIComponent(token)}`);
    ...
    async patch(token, patch) {
      const res = await fetch(`${url}/${encodeURIComponent(token)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(patch) });
      const body = await safeJson(res) as (ApiBody & { data?: Record<string, unknown> }) | null;
      return { ok: res.ok && Boolean(body?.success), session: body?.data ?? null, error: pickErrorMessage(body), errorCode: pickErrorCode(body) };
    },
    async rate(token) {
      const res = await fetch(`${url}/${encodeURIComponent(token)}/rate`, { method: 'POST' });
      const body = await safeJson(res) as (ApiBody & { data?: Record<string, unknown> }) | null;
    ...
    async issue(token) {
      const res = await fetch(`${url}/${encodeURIComponent(token)}/issue`, { method: 'POST' });
    ...
    async fork(token) {
      const res = await fetch(`${url}/${encodeURIComponent(token)}/fork`, { method: 'POST' });
