...
export function Step3VehicleSpecsSection({
  ...
  filteredVehicleTypeOptions, canUseTrimLookup, isElectric, isCaravan, isMotorcycle,
  onChange, onUserChange,
}: Props) {
  const [engineSizeEditing, setEngineSizeEditing] = useState(false);
  const [engineSizeUi, setEngineSizeUi] = useState('');
  const [batteryKWhEditing, setBatteryKWhEditing] = useState(false);
  const [batteryKWhUi, setBatteryKWhUi] = useState('');
  ...
  return (
    <SectionCard title="" icon={<Car className="w-6 h-6" />}>
      <div className="grid grid-cols-1 gap-0 md:grid-cols-2 md:gap-6">
        ...
        {/* ABY-321 — a motorbike is never a cabriolet; hide the question and
            let Step3VehicleCover pin the value to 'No'. */}
        {!isMotorcycle && (
      ...
      <div className="grid grid-cols-1 gap-0 md:grid-cols-2 md:gap-6">
        ...
        <div className={specAutofillHighlights.engineSize ? 'step3-autofill-flash' : ''}>
          {isElectric ? (
            <FormField label="Battery Capacity (kWh)" required error={errors.batteryKWh} tooltip="Enter the total battery capacity of the electric vehicle in kilowatt-hours (kWh)." fieldKey="batteryKWh">
              <Input type="text" inputMode="numeric" min={5} max={400}
                value={batteryKWhEditing ? batteryKWhUi : (data.batteryKWh != null ? String(data.batteryKWh) : '')}
                onFocus={() => { setBatteryKWhEditing(true); setBatteryKWhUi(data.batteryKWh != null ? String(data.batteryKWh) : ''); }}
                onBlur={() => { setBatteryKWhEditing(false); setBatteryKWhUi(data.batteryKWh != null ? String(data.batteryKWh) : ''); }}
                onChange={(e) => { const v = String(e.target.value || '').replace(/[^\d]/g, '').slice(0, 3); setBatteryKWhUi(v); const n = parseInt(v, 10); onUserChange('batteryKWh', Number.isFinite(n) ? n : null); }}
                error={!!errors.batteryKWh} showValid={!errors.batteryKWh && data.batteryKWh != null && Number(data.batteryKWh) > 0} placeholder="e.g. 77" />
            </FormField>
          ) : (
