import { Bike, Bus, Car, Hash, Loader2, ScanLine, Truck } from 'lucide-react';
...
export function Step3VehicleIdentitySection(props: Props) {
  ...
  const identifyByOptions: SegmentedOption<'registration' | 'vin'>[] = [
  ...
  const vehicleTypeOptions: SegmentedOption<VehicleTypeKind>[] = [
    ...
    { id: 'caravan', label: 'Caravan', icon: Bus },
    { id: 'van', label: 'Van', icon: Truck },
  ];

  const [idMode, setIdMode] = useState<'registration' | 'vin'>(() => {
    const hasVin = Boolean(String(props.data.vin || '').trim());
    const hasReg = Boolean(String(props.data.registrationNumber || '').trim());
    // ABY-105 — registration is the canonical default: per the
    // marker.io report, customers expect the segmented switch to land
    // on Registration unless a VIN is the only identifier they have on
    // file. Only flip to VIN when a VIN is present AND no
    // registrationNumber is.
    if (hasVin && !hasReg) return 'vin';
    return 'registration';
  });

  // ABY-149 — when the user explicitly switches to VIN mode, suppress the
  // "both blank → snap to registration" recovery until they either type a
  // VIN or switch back to registration themselves. A one-shot flag is NOT
  // enough because the parent re-renders asynchronously after clearing
  // registrationNumber, causing a second effect run where the flag has
  // already been consumed. We keep it true until the user's action is
  // reflected in data (vin non-empty) or they explicitly go back.
  const suppressSnapBackRef = useRef(false);

  useEffect(() => {
    const hasVin = Boolean(String(props.data.vin || '').trim());
    const hasReg = Boolean(String(props.data.registrationNumber || '').trim());
    if (hasVin && !hasReg && idMode !== 'vin') setIdMode('vin');
    if (hasReg && !hasVin && idMode !== 'registration') setIdMode('registration');
    // ABY-105 — when BOTH fields are blank snap back to the canonical
    // 'registration' default, UNLESS the user just deliberately switched
    // to VIN (suppression flag is still set). Clear the flag only when
    // the user types a VIN so the next blank-check is honest.
    if (hasVin) {
      suppressSnapBackRef.current = false;
    }
    if (!hasVin && !hasReg && idMode !== 'registration' && !suppressSnapBackRef.current) {
      setIdMode('registration');
    }
  }, [idMode, props.data.registrationNumber, props.data.vin]);
    ...
    <SectionCard title="Your vehicle" icon={<Car className="w-6 h-6" />}>
      ...
      <div className="grid grid-cols-1 gap-0 md:grid-cols-3 md:gap-6">
        <div className="mb-6 md:col-span-1">
          <label className="block mb-2 text-[14px] font-semibold text-slate-400 tracking-tight">Identify vehicle by</label>
        <SegmentedSwitch
          value={idMode}
          options={identifyByOptions}
          onChange={(nextMode) => {
            setIdMode(nextMode);
            if (nextMode === 'registration') {
              suppressSnapBackRef.current = false;
              props.onVinChange('');
            } else {
              // ABY-149 — set the suppression flag BEFORE clearing
              // registrationNumber so the effect doesn't snap back to
              // 'registration' while both fields are transiently blank
              // (the parent re-renders asynchronously after clearing).
              suppressSnapBackRef.current = true;
              props.onUserChange('registrationNumber', '');
            }
          }}
          className="w-full"
        />
        ...
        <div className="md:col-span-2">
          <FormField
            label={idMode === 'registration' ? 'Registration Number' : 'VIN'}
            error={idMode === 'registration' ? props.errors.registrationNumber : props.errors.vin}
            tooltip={idMode === 'registration' ? 'Most customers use registration number.' : 'VIN can identify the vehicle when registration is unavailable.'}
            fieldKey={idMode === 'registration' ? 'registrationNumber' : 'vin'}
          >
            <Input
              value={idMode === 'registration' ? String(props.data.registrationNumber || '') : String(props.data.vin || '')}
              onChange={(e) => {
                if (idMode === 'registration') {
                  props.onUserChange('registrationNumber', String(e.target.value || '').toUpperCase());
                } else {
                  const nextVin = String(e.target.value || '').toUpperCase().replace(/\s+/g, '');
                  props.onVinChange(nextVin);
                }
              }}
              onBlur={idMode === 'vin' ? props.onVinBlurLookup : undefined}
              error={idMode === 'registration' ? !!props.errors.registrationNumber : !!props.errors.vin}
              placeholder={idMode === 'registration' ? 'e.g. KAA123' : 'Enter VIN (17 characters)'}
            />
          </FormField>
        </div>
      </div>

      {idMode === 'vin' && props.vinLookupLoading ? (
      ...
      {idMode === 'vin' && props.vinLookupMessage ? (
