...
export function Step3VehicleCover() {
  ...
  // Editing state for engine/battery/vehicleValue lives in Step3VehicleSpecsSection.

  // Electric vehicles do not have a combustion engine displacement. We
  // use `engineSize === 1` as the canonical EV sentinel — matching the
  // server-side normalizer `normalizeElectricVehicleCompatibility` in
  // `backend/products/motor/quotes/quoteDataGuards.ts`. The canonical
  // capacity field is `batteryKWh` (added ABY-182). Switching away
  // from Electric clears the sentinel so the next ICE value is
  // captured fresh and not silently mis-rated as a 1cc engine.
  const isElectric = String(data.fuelType || '').trim() === 'Electric';
  useEffect(() => {
    if (isElectric) {
      if (data.engineSize !== 1) {
        onChange('engineSize', 1);
      }
    } else if (data.engineSize === 1) {
      // Clear the EV sentinel so the next ICE value is captured fresh
      // and not silently mis-rated as a 1cc engine. `resetField` is
      // RHF's canonical clear-to-default — sidesteps the strict
      // `engineSize: number` schema without an unsafe coercion.
  ...
  // ABY-321 — a motorbike is never a cabriolet. The Cabrio question is
  // hidden for motorbikes (see Step3VehicleSpecsSection), so pin the
  // canonical value to 'No' to satisfy the required-field validator.
  useEffect(() => {
    if (!isMotorcycle) return;
    if (data.cabrio === 'No') return;
    onChange('cabrio', 'No');
  }, [data.cabrio, isMotorcycle, onChange]);
