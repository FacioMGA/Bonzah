...
const SANDBOX_TENANT_SLUG = 'abbeygate-config-mcp-sandbox';
...
export const LaunchReadinessPanel: React.FC<Props> = ({ summary, onRefresh }) => {
    ...
    async function publish() {
        if (!summary) return;
        const confirmationText = `PUBLISH ${summary.productName} TO SANDBOX`;
        if (!confirm(`Confirm sandbox publish? Type:\n\n  ${confirmationText}`)) return;
        setBusy(true);
        try {
            const envelope = await configurationApi.publishToSandbox({
                draftId: summary.draftId,
                sandboxTenantSlug: SANDBOX_TENANT_SLUG,
                confirmationText,
            });
            if (envelope.success) {
                setPublishMessage(envelope.result.summary);
            } else {
                setPublishMessage(`${envelope.error.code}: ${envelope.error.message}`);
            }
            onRefresh();
        } finally {
            setBusy(false);
    ...
    return (
        <div className="p-4 space-y-4 text-xs">
            ...
            <section className="space-y-2 pt-3 border-t border-slate-200">
                <h3 className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Sandbox publish</h3>
                <p className="text-[11px] text-slate-500">
                    Available once the draft reaches status <code>simulated</code>. Targets the demo
                    SYNTHETIC tenant <code>{SANDBOX_TENANT_SLUG}</code>.
                </p>
                <Button
                    onClick={publish}
                    disabled={busy || summary.status !== 'simulated'}
                    className="text-xs w-full"
                >
                    Publish to sandbox
                </Button>
                {publishMessage && (
                    <div className="rounded p-2 bg-blue-50 text-blue-800 text-[11px]">{publishMessage}</div>
                )}
            </section>
        </div>
    );
