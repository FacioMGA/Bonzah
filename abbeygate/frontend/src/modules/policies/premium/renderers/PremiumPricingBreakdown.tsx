...
type PremiumPricingBreakdownProps = {
  ...
  patchQuoteData: (nextQuoteData: Record<string, unknown>) => void;
              ...
              <>
                ...
                {(() => {
                  ...
                  const bdLines = Object.entries(bd)
                    .filter(([key, v]) => typeof v === 'number' && v !== 0 && key !== 'lines')
                    .map(([key, amount]) => ({
                      key,
                      label: key.replace(/([A-Z])/g, ' $1').replace(/^./, s => s.toUpperCase()).replace(/Final$/, '').trim(),
                ...
                {(() => {
                  ...
                  const updateAdjustments = (newAdj: Adjustment[]) => {
                    patchQuoteData({ ...(qd || {}), uwAdjustments: normalizeAdjustments(newAdj), uwAdjustment: null });
                  };
                  const persistAdjustmentsAndReRate = (newAdj: Adjustment[], rerate: boolean) => {
