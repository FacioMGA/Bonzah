...
export const programsApiClient = {
    ...
    async getProgramUwConfig<T extends ProgramUwConfig = ProgramUwConfig>(id: string) {
        return http.request<T>(`programs/${id}/uw-config`);
    },

    async saveProgramUwConfig<T extends ProgramUwConfig = ProgramUwConfig>(id: string, data: T) {
        return http.request<T>(`programs/${id}/uw-config`, {
            method: 'PUT',
            body: JSON.stringify(data),
        });
    ...
    async publishProgramRatingModel(id: string) {
