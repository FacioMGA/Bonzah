...
const platformLogo = '/assets/branding/logo-white.png';
  ...
  const verifyEmailAndLogin = async () => {
    ...
    try {
      ...
      if (token) {
        ...
        onLogin(token, user);
        if (explicitNextUrl) window.location.assign(explicitNextUrl);
      } else if (password) {
      ...
      <div className="w-full max-w-[440px] space-y-8 animate-in fade-in duration-500">
        <div className="text-center">
          <img
            src={platformLogo}
            alt="MGA platform"
            className="relative -top-4 h-14 w-auto mx-auto mb-6 select-none"
            draggable={false}
          />
          ...
          <p className="text-slate-500 text-sm font-medium">
            {isSignUp ? 'Join the MGA platform' : 'Welcome back to the platform'}
          </p>
          {notice && (
