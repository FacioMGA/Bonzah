...
export function makeQuoteStartPage(productCode: string, firstStep: string) {
  ...
  return function QuoteStartPage() {
    ...
    // ADR-0046 — do not create a session for a product whose online
    // questionnaire is switched off for this tenant. Admins bypass.
    const { channel, loading: channelLoading } = useProductChannel(productCode);
    const entryBlocked = channel ? !channel.questions : false;

    useEffect(() => {
      if (channelLoading || entryBlocked) return;
      let cancelled = false;
      (async () => {
        try {
          ...
          navigate(`/quote/${publicId}?product=${productCode}&step=${firstStep}`, { replace: true });
        } catch (e) {
    ...
    }, [navigate, attempt, channelLoading, entryBlocked]);
      ...
      return (
        <div className="min-h-screen flex items-center justify-center bg-brand-canvas text-slate-700 px-6">
          <div className="ui-card ui-card-pad max-w-lg w-full text-center">
            <div className="text-sm font-black text-slate-900">Online quotes are not available for this product right now</div>
            <div className="text-xs text-slate-500 mt-2">
              Please contact Abbeygate and an advisor will be happy to help you with a quote.
            </div>
          </div>
        </div>
      );
