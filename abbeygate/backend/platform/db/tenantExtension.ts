...
const TENANT_SCOPED_MODELS = new Set([
  ...
  'ProductLaunchDraft',
  // Product channel switches (ADR-0046): per-tenant x product public journey gates.
...
export async function applyTenantInjection(ctx: OperationContext): Promise<unknown> {
  ...
  if (baseClient && !tenantTransactionScope.getStore()) {
    return baseClient.$transaction(async () => {
      await baseClient.$executeRaw`SELECT set_config('app.operating_tenant_id', ${tenantId}, true)`;
      return query(args);
