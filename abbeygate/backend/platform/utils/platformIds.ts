...
export async function generateQuoteId(productType?: string | null) {
  return await tenantScopedPrisma.$transaction((tx) => reserveNextQuoteId(tx as unknown as Tx, productType));
}

export async function generatePolicyId(productType?: string | null, origin: IssueOrigin = 'ONLINE') {
  return await tenantScopedPrisma.$transaction((tx) => reserveNextPolicyId(tx as unknown as Tx, productType, origin)); // TODO(FAC-9035): owner=platform-tenant expires=2026-12-31 deletionPR=typed-tenant-tx tenantScopedPrisma extension tx vs Tx union — pre-existing cast, identical to generateQuoteId above.
}
