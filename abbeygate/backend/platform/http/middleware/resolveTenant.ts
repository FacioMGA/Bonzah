 ...
 *   3. Request host, e.g. abbeygate-cy.facio.io -> abbeygate-cy
...
type TenantRow = Awaited<ReturnType<typeof prisma.tenant.findUniqueOrThrow>>;

function rowToConfig(row: TenantRow): TenantConfig {
  const ipt = row.iptJson as { rate?: number; flatFee?: number } | null ?? {};
  return {
    id:            row.id,
    ...
    brandLogo: {
      white: (row.brandLogos as Record<string, string> | null)?.white ?? '',
      blue:  (row.brandLogos as Record<string, string> | null)?.blue  ?? '',
    },
...
// Host → tenant-slug mapping. The slug identifies the jurisdiction tenant, NOT
// the environment: the same slug (e.g. abbeygate-cy) exists in every
// deployment. Prod vs test isolation is enforced by the deployment + database +
// secrets behind the hostname (which ingress answers on which host), never by
// this map. Listing prod and staging hosts side by side is therefore safe: each
// environment's ingress only serves its own hostnames.
const HOST_TENANT_SLUGS: Readonly<Record<string, string>> = {
  // Production public domains (abbeygate.com). CY/PT/GR go live on abbeygate.com;
  // ES has no online purchase route yet, so it stays on facio.io only.
  'cy.abbeygate.com': 'abbeygate-cy',
  'pt.abbeygate.com': 'abbeygate-pt',
  'gr.abbeygate.com': 'abbeygate-gr',
  // Test / staging domains (staging-marked so a live host is never mistaken
  // for a test host, and vice versa).
  'cy.staging.abbeygate.com': 'abbeygate-cy',
  'pt.staging.abbeygate.com': 'abbeygate-pt',
  'gr.staging.abbeygate.com': 'abbeygate-gr',
  // Legacy facio.io hosts — kept live on the production deployment through the
  // domain transition so existing links/bookmarks keep working.
  'abbeygate-cy.facio.io': 'abbeygate-cy',
  'abbeygate-pt.facio.io': 'abbeygate-pt',
  'abbeygate-gr.facio.io': 'abbeygate-gr',
  'abbeygate-es.facio.io': 'abbeygate-es',
};
...
function resolveSlugFromHost(req: Request): string | null {
  ...
  const host = normalizeHost(forwardedHost) ?? normalizeHost(directHost);
  if (!host) return null;
  return HOST_TENANT_SLUGS[host] ?? null;
}
