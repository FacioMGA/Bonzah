...
const REQUIRED_STATIC_PDFS: Array<{ name: string; relPath: string }> = [
  ...
  {
    name: 'home_ipid_greece_static_pdf',
    relPath: '../../products/home/documents/static/LloydsHome_IPID_GR.pdf',
  },
  {
...
async function readLatestSettingsWithTimeout(timeoutMs = 3000): Promise<Record<string, unknown> | null> {
  try {
    ...
    const settingsPromise = prisma.settings.findFirst({ orderBy: { updatedAt: 'desc' } })
...
export async function validateStartupConfig(): Promise<void> {
  ...
  const bonzahMode = String(process.env.BONZAH_EXECUTION_MODE || 'simulation').trim().toLowerCase();
  if (!['simulation', 'insillion'].includes(bonzahMode)) {
    errors.push("[StartupValidation] BONZAH_EXECUTION_MODE must be 'simulation' or 'insillion'.");
  }
  if (bonzahMode === 'insillion') {
    const baseUrl = String(process.env.BONZAH_INSILLION_BASE_URL || 'https://bonzah.sb.insillion.com').trim();
    ...
    const timeoutMs = Number(process.env.BONZAH_INSILLION_TIMEOUT_MS || 10_000);
    if (!baseUrl.startsWith('https://')) errors.push('[StartupValidation] BONZAH_INSILLION_BASE_URL must use HTTPS.');
    if (!username || !password) errors.push('[StartupValidation] Insillion mode requires BONZAH_INSILLION_USERNAME and BONZAH_INSILLION_PASSWORD.');
    if (!Number.isFinite(timeoutMs) || timeoutMs < 1000 || timeoutMs > 60_000) errors.push('[StartupValidation] BONZAH_INSILLION_TIMEOUT_MS must be between 1000 and 60000.');
  }

  // 0) JWT_SECRET and QUOTE_TOKEN_SECRET must each be set to a strong
  // secret in production. They live in separate keyspaces (ADR-0019 /
  // PR 2): rotating one does not invalidate the other. Local-dev
  // placeholders are rejected from prod via the shared deny-set.
  const jwtSecret = String(process.env.JWT_SECRET || '').trim();
  const quoteTokenSecret = String(process.env.QUOTE_TOKEN_SECRET || '').trim();
  const insecureSecretPlaceholders = new Set([
    'dev-jwt-secret-change-me',
    'local-only-change-me-with-openssl-rand-hex-32',
  ]);
  if (isProd() && (!jwtSecret || insecureSecretPlaceholders.has(jwtSecret))) {
    errors.push('[StartupValidation] JWT_SECRET must be set to a strong secret in production. Local-dev placeholders are rejected.');
  }
  if (isProd() && (!quoteTokenSecret || insecureSecretPlaceholders.has(quoteTokenSecret))) {
    errors.push('[StartupValidation] QUOTE_TOKEN_SECRET must be set to a strong secret in production. Local-dev placeholders are rejected. Per ADR-0019 the legacy JWT_SECRET fallback is gone.');
  }
  if (isProd() && jwtSecret && quoteTokenSecret && jwtSecret === quoteTokenSecret) {
    errors.push('[StartupValidation] JWT_SECRET and QUOTE_TOKEN_SECRET must differ in production. Per ADR-0019 the keyspaces are intentionally separated; reusing the same secret defeats independent rotation.');
  }
  ...
  if (isProd() && !String(process.env.OPENAI_API_KEY || '').trim() && !allowStubBehaviorEmbeddings) {
