...
const prismaMock = {
  ...
  // ADR-0046 — the checkout handler now resolves the product payment channel.
  // This tenant has motor payment enabled, so the gate passes and we exercise
  // the idempotency / redirect-URL path under test.
...
describe('paymentsCardcorp checkout idempotency', () => {
  beforeEach(() => {
    ...
    prismaMock.productChannelSetting.findFirst.mockResolvedValue({
