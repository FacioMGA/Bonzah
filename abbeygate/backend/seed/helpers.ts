...
import { calculateAutoInsuranceQuoteResponse } from '../products/motor/pricing/autoInsuranceCalculator.js';
import { evaluateMotorUwAutomation } from '../products/motor/underwriting/motorUwAutomation.js';
...
export { calculateAutoInsuranceQuoteResponse };
...
export async function createComputedAutoQuote(args: {
  ...
  const quoteResponse = calculateAutoInsuranceQuoteResponse(quoteData, undefined, { reference: policyNumber, currency: 'EUR' });
  const uwDecision = evaluateMotorUwAutomation(quoteData);
