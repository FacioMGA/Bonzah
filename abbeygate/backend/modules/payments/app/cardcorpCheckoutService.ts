...
export async function createCardcorpAutoCheckout(args: {
  ...
  if (idempotencyKey) {
    const existing = await tenantScopedPrisma.payment.findFirst({
      where: { policyId: policy.id, provider: 'CARDCORP', idempotencyKey },
  ...
  const payment = await tenantScopedPrisma.payment.create({
    data: {
      policyId: policy.id,
      provider: 'CARDCORP',
  ...
  await tenantScopedPrisma.$transaction(async (_tx) => {
    ...
    if (String(policy.status || '').toUpperCase() === 'QUOTED') {
