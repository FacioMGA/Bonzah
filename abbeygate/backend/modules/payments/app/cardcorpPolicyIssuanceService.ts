...
import { parseCoverageSelectionSnapshot } from '../../policy/app/coverageSelectionContract.js';

type ParseRecord = (value: unknown) => Record<string, unknown>;
type JsonStringify = (value: unknown) => unknown;
...
export async function runCardcorpPaidIssuance(args: {
...
}): Promise<{ selectedBundleId: string }> {
  ...
  const issuance = await tenantScopedPrisma.$transaction(async (_tx) => {
    ...
    const existingQuoteId = isReservedQuoteId(candidate.policyNumber)
      ...
      : undefined;
    const policyBusinessId = shouldReassignPolicyNumberAtIssuance(productType, candidate.policyNumber)
    ...
    let binderIdToUse: string | undefined = candidate.binderId ?? undefined;
    if (!binderIdToUse) {
      const defaultBinder = await tx.binder.findFirst({
        where: { status: 'ACTIVE' },
    ...
    const boundBinder = binderIdToUse
      ? (binderIdToUse === candidate.binderId
          ? candidate.binder
          : await tx.binder.findUnique({ where: { id: binderIdToUse }, select: { id: true, umr: true } }))
      : null;
  ...
  return { selectedBundleId: issuance.selectedBundleId };
}
