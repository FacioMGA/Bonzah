...
import { enforceProductChannelGate } from '../../policy/http/productChannelGate.js';
...
router.post('/auto/:policyId/checkout', paymentsAuditLog, async (req, res) => {
  try {
    ...
    // ADR-0046 — refuse online payment when the product's payment channel is
    ...
    if (!(await enforceProductChannelGate(req, res, String(policy.productType || ''), 'payment'))) return;
  ...
  try {
    ...
    return res.json({
      ...
      data: {
        ...
        policyStatus: status.ok ? 'PAID' : 'UNPAID',
      },
    });
  } catch (error) {
