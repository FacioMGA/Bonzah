...
function checkOverlayKeysAllowlist(delta: DraftDelta, issues: ValidationIssue[]): void {
    // Maps DraftDelta slots → the Program.metadata keys they publish into.
    // Any slot whose write target is outside the allowlist would be a
    // contract-spine violation (ADR-0038). The runtime check here is
    // defense-in-depth; the static guard
    // `tools/quality/check-configuration-overlay-bounds.mjs` is the
    // primary enforcement.
