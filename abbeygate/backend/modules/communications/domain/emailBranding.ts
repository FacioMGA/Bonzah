...
export type BrandJurisdictionKey = 'CY' | 'PT' | 'GR' | 'ES';
...
export interface BrandSignature {
  ...
  /** Optional public website (display label) — e.g. "Abbeygate.cy". */
  websiteLabel?: string;
...
// ────────────────────────────────────────────────────────────────────────────
// Static signature data (per jurisdiction). Source: regulatory copy.
// ────────────────────────────────────────────────────────────────────────────

const CY_SIGNATURE: BrandSignature = {
  jurisdiction: 'Cyprus',
  legalLines: [
    "Abbeygate is a trading name of Abbeygate Insurance Brokers LTD,",
    "Lloyd's Coverholder registered and regulated in Cyprus.",
  ],
  addressLines: [
    'Head Office: Shop 1, Mesogi Avenue,',
    'Paphos 8280, Cyprus',
  ],
  regulatorLine: 'Regulated by Insurance Companies Control Service (ICCS)',
  websiteLabel: 'Abbeygate.cy',
  websiteUrl: 'https://abbeygate.cy',
  phones: [
    {
      label: 'WhatsApp / Viber — New Business',
      display: '+357 97 612602',
      tel: '+35797612602',
      whatsapp: '35797612602',
    },
    {
      label: 'WhatsApp / Viber — Renewals',
      display: '+357 99 218015',
      tel: '+35799218015',
      whatsapp: '35799218015',
    },
    {
      label: 'For queries please call',
      display: '+357 26 819175',
      tel: '+35726819175',
    },
  ],
};

const PT_SIGNATURE: BrandSignature = {
  jurisdiction: 'Portugal',
  legalLines: [
    'Abbeygate is a trading name of Abbeygate Insurance Brokers LTD,',
    'Sucursal em Portugal — corporate number (NIPC) 980831970.',
  ],
  addressLines: [
    'Estrada Regional 125, Loja 149A,',
    'Boliqueime, 8100-069, Loulé, Portugal',
  ],
  regulatorLine: 'Registered numbers 922039390 and 923047966',
  phones: [
    {
      label: 'WhatsApp / Viber — New Business',
      display: '+351 968 127522',
      tel: '+351968127522',
      whatsapp: '351968127522',
    },
    {
      label: 'WhatsApp / Viber — Renewals',
      display: '+351 925 602100',
      tel: '+351925602100',
      whatsapp: '351925602100',
    },
    {
      label: 'For queries please call',
      display: '+351 289 369 254',
      tel: '+351289369254',
    },
  ],
};

const ES_SIGNATURE: BrandSignature = {
  jurisdiction: 'Spain',
  legalLines: [
    "Abbeygate is a trading name of Abbeygate Insurance Brokers LTD,",
    "Lloyd's Coverholder registered and regulated in Cyprus.",
...
const GR_SIGNATURE: BrandSignature = {
  jurisdiction: 'Greece',
  legalLines: [
    'Abbeygate is a trading name of Abbeygate Insurance Brokers Ltd,',
    'registered and regulated in Cyprus.',
  ],
  addressLines: [
    'Head Office: Shop 1, Mesogi Avenue,',
    'Paphos 8280, Cyprus',
  ...
  phones: [
    {
      label: 'For queries please call',
      display: '+30 211 2345 774',
      tel: '+302112345774',
    },
    {
      label: 'For queries please call',
      display: '+44 7593 621183',
      tel: '+447593621183',
    },
  ],
...
const SIGNATURES: Record<BrandJurisdictionKey, BrandSignature> = {
  CY: CY_SIGNATURE,
  PT: PT_SIGNATURE,
  ES: ES_SIGNATURE,
  GR: GR_SIGNATURE,
...
/**
 * Pure resolver — turns a tenant snapshot into a {@link BrandProfile}. Keeps
 * fallbacks deterministic so that a missing logo URL or unknown jurisdiction
 * still produces a complete, regulator-compliant signature.
 */
export function buildBrandProfile(input: BuildBrandProfileInput): BrandProfile {
  const jurisdictionKey = normalizeJurisdiction(input.countryCode);
  const brandName = (input.brokerName && input.brokerName.trim()) || 'Abbeygate';
  const baseUrl = stripTrailingSlash(input.publicBaseUrl) || 'https://abbeygate.cy';
  const logoUrl = (input.whiteLogoUrl && input.whiteLogoUrl.trim())
    || `${baseUrl}/assets/branding/logo-white.png`;
  const colors = { ...DEFAULT_COLORS, ...(input.colors || {}) };
  const signature = SIGNATURES[jurisdictionKey];
  return {
    brandName,
    jurisdictionKey,
    logoUrl,
...
/** Fallback profile used when no tenant context is available (CLI / tests). */
...
function normalizeJurisdiction(raw: string | undefined | null): BrandJurisdictionKey {
  const code = String(raw || '').trim().toUpperCase();
  if (code === 'CY' || code === 'PT' || code === 'GR' || code === 'ES') return code;
  return 'CY';
}
