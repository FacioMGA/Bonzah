...
import { buildBrandProfile } from '../../domain/emailBranding.js';
import { assertSyntheticRecipientsAllowed } from '../syntheticRecipientAllowlist.js';
import { CONTACT_EMAIL_BY_COUNTRY, contactEmailForCountry, type CountryCode } from '../../../../platform/tenant/tenantConfig.js';
...
/**
 * Sender display name for the "From" header. Customers must see the
 * Abbeygate brand (per-tenant), never the platform vendor name that a
 * bare `no-reply@…` address would otherwise inherit from the SendGrid
 * verified-sender default. Resolved from the tenant country code the
 * customer-email pipeline already stamps onto `externalRefs`; falls back
 * to the `EMAIL_FROM_NAME` env override, then to the canonical broker
 * brand. The address itself is unchanged (still env-driven) so sender
 * verification is unaffected — only the display label changes.
 */
function resolveFromDisplayName(refs: ExternalRefsView): string {
    ...
    return buildBrandProfile({ countryCode, publicBaseUrl: '', fromEmail: '' }).brandName;
}
...
function isCountryCode(value: string): value is CountryCode {
    return Object.prototype.hasOwnProperty.call(CONTACT_EMAIL_BY_COUNTRY, value);
}

/**
 * Reply-To for customer-facing sends (ABY-440). From stays on the
 * verified no-reply sender; replies route to the tenant contact inbox
 * owned by `contactEmailForCountry`.
 */
function resolveReplyTo(refs: ExternalRefsView): string | undefined {
    const countryCode = String(refs.tenantCountryCode || '').trim().toUpperCase();
    if (!isCountryCode(countryCode)) return undefined;
    return contactEmailForCountry(countryCode);
}
