...
router.post('/', async (req, res) => {
    try {
        ...
        const { reserveNextPolicyId, reserveNextCertificateNumber } = await import('../../../platform/utils/platformIds.js');
        ...
        const result = await tenantScopedPrisma.$transaction(async (_tx) => {
          const tx = _tx as Prisma.TransactionClient;
            const policyBusinessId = await reserveNextPolicyId(tx, policy.productType, 'MANUAL');
            const certificateNumber = await reserveNextCertificateNumber(tx);
