...
import { jsonParse, jsonStringify, newPublicSessionToken } from '../app/shared.js';
...
export function registerPolicyMutationRoutes(router: Router) {
   ...
   * clears the referral "red/yellow lane" by moving lifecycle status to QUOTED (green lane),
   * and persists approval metadata into policyStateCurrent.snapshot.
   */
  router.post('/:id/manual-uw-approval', policyAuditLog, async (req, res) => {
    try {
      ...
      const updated = await tenantScopedPrisma.$transaction(async (_tx) => {
        const tx = _tx as Prisma.TransactionClient;
        const state = await tx.policyStateCurrent.findUnique({ where: { policyId } });
        const prev = parseSnapshotRecord(state ? jsonParse(state.snapshot) : {});
        ...
        const nextSnapCreate: WithoutTenantScope<Prisma.PolicyStateCurrentUncheckedCreateInput> = {
          policyId,
        ...
        await tx.policyStateCurrent.upsert({
          ...
          create: nextSnapCreate as Prisma.PolicyStateCurrentUncheckedCreateInput,
        });

        const nextStatus = String(policy.status || '').toUpperCase() === 'REFERRAL' ? 'QUOTED' : String(policy.status || '').toUpperCase();
        if (nextStatus !== String(policy.status || '').toUpperCase()) {
          await transitionPolicyLifecycle({
            tx,
            policyId,
            to: nextStatus as Parameters<typeof transitionPolicyLifecycle>[0]['to'],
            actorId: actor?.id || 'system',
            actorType: actor?.role ? 'USER' : 'SYSTEM',
            reasonCode: 'UW_MANUAL_APPROVAL',
            correlationId: req.correlationId,
            data: { source: 'policies/manual-uw-approval' },
          });
        }
        const p = await tx.policy.findUniqueOrThrow({
          where: { id: policyId },
          select: { id: true, status: true },
        });
  ...
  try {
    ...
    const result = await tenantScopedPrisma.$transaction(async (_tx) => {
      ...
      try {
        ...
        // Unit tests/mocks or deployments without the reservation table should still function.
