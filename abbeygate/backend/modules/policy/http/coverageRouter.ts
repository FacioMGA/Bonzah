...
import { getOrInitProgramMbeProductConfig } from '../app/mbeInterop.js';
...
} from '../app/coverageSelectionContract.js';
...
export function registerPolicyCoverageRoutes(router: Router) {
  router.get('/:id/coverage-options-view', policyCoverageAuditLog, async (req, res) => {
    try {
      ...
      const [policy, currentState, activeInstances] = await Promise.all([
        tenantScopedPrisma.policy.findUnique({ where: { id: policyId }, select: { id: true, programId: true, productType: true, quoteData: true } }),
      ...
      const contract = resolveEffectiveCoverageContract({
        ...
        programId,
        boInitialized: parseCoverageSelectionSnapshot(stored).bo_initialized === true,
      });
  ...
  router.post('/:id/coverage-selection/init', policyCoverageAuditLog, async (req, res) => {
    try {
      ...
      const stored = parseRecord(snapshot.coverageSelection);
      const effectiveQuoteData = resolveEffectiveQuoteData({ snapshot, policyQuoteData: policy.quoteData });
      const normalizedStored = parseCoverageSelectionSnapshot(stored);
      ...
      const cfg = await getOrInitProgramMbeProductConfig(programId);
      const initContract = resolveEffectiveCoverageContract({
        productType: String(policy.productType || ''),
        quoteData: effectiveQuoteData,
        cfg,
        storedSelection: {
      ...
      const initSnapshot = normalizeCoverageSelectionSnapshot({
        value: {
          ...normalizedStored,
          programId,
  ...
  router.put('/:id/coverage-selection', policyCoverageAuditLog, async (req, res) => {
    try {
      ...
      const policy = await tenantScopedPrisma.policy.findUnique({ where: { id: policyId }, select: { id: true, programId: true, productType: true, status: true } });
      if (!policy) {
        return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Policy not found' } });
      }
      ...
      if (!programId) {
        ...
        const nextSnapshot = normalizeCoverageSelectionSnapshot({
          value: {
      ...
      const cfg = await getOrInitProgramMbeProductConfig(programId);
      const normalizedPayload = normalizeCoverageSelectionSnapshot({
        value: {
          ...parseCoverageSelectionSnapshot(payload),
          selected: toBooleanRecord(payload.selected),
          params: parseRecord(payload.params),
