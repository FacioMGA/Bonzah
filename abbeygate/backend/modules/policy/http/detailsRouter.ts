...
import { buildMagicBSectionsForSchedule, MagicB, normalizeProgramMbeProductConfig } from '../app/mbeInterop.js';
import { getTenantConfig } from '../../../platform/tenant/tenantConfig.js';
import { buildProductDocumentViewModel, productAdapterExists } from '../app/productRegistryService.js';
import { jsonParse } from '../app/shared.js';
import { normalizeProductKit } from '../app/programsInterop.js';
    ...
    try {
      ...
      if (includeDocView && productAdapterExists(docViewProductType)) {
        try {
          ...
          const programFindFirst = getMethod(programDelegate, 'findFirst');
          const [binder, program, activeEndorsements] = await Promise.all([
            policyData.binderId
              ? tenantScopedPrisma.binder.findUnique({ where: { id: policyData.binderId } })
              : Promise.resolve(null),
            policyData.programId
              ? Promise.resolve(programFindUnique?.({ where: { id: policyData.programId } }) ?? null)
              : Promise.resolve(programFindFirst?.({ where: { status: 'ACTIVE' }, orderBy: { updatedAt: 'desc' } }) ?? null),
          ...
          const programRecord = parseRecord(program);
          const meta = parseRecord(programRecord.metadata);
          const kit = normalizeProductKit(meta.productKit);
          const brand = kit.brand;

          const normalizedMbeCfg = normalizeProgramMbeProductConfig(meta.mbeProductConfig, {
            productType: String(policyData.productType || ''),
            programCode: String(meta.programCode || `abbeygate_${String(policyData.productType || '').trim().toLowerCase()}`),
          });
          ...
          const baseApplied = resolveEffectiveCoverageContract({
            ...
            programId: String(policyData.programId || programRecord.id || ''),
