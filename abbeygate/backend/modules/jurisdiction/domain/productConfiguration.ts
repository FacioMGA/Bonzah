...
export type JurisdictionCountryCode = 'CY' | 'PT' | 'GR' | 'ES' | 'BE' | 'NL' | 'IT' | 'FR' | 'MT';
export type JurisdictionProductCode = 'MOTOR' | 'HOME' | 'TRAVEL' | 'HEALTH' | 'BUSINESS' | 'OPEN_MARKET';
...
export type JurisdictionResolverInput = {
  ...
  tenant?: { countryCode?: string | null; country?: string | null; tenantSlug?: string | null } | null;
...
function normalizeProductCode(raw: unknown): JurisdictionProductCode {
  ...
  if (value === 'MOTOR' || value === 'HOME' || value === 'TRAVEL' || value === 'HEALTH' || value === 'BUSINESS' || value === 'OPEN_MARKET') return value;
...
function resolveCountryCode(
  ...
  const programMeta = asRecord(input.program?.metadata);
  const binderConfig = asRecord(input.binder?.config);
  const candidates = [
    input.source?.countryCode,
    programMeta.countryCode,
    asRecord(programMeta.jurisdiction).countryCode,
  ...
  throw new ProductConfigurationError({
    ...
    reason: suppliedCountry
      ...
      : 'Country code not supplied by Program, Binder, Tenant, or Source',
  });
}
...
function makeConfig(args: {
  ...
  const preset =
    args.productCode === 'TRAVEL'
      ? JURISDICTION_TRAVEL_DOCUMENT_PRESETS[args.countryCode]
...
const GR_GREEN_CARD: GreenCardJurisdictionConfig = {
  bureauName: 'Hellenic Motor Insurers Bureau',
  countryCode: 'GR',
  codePrefix: 'GR',
  insurerCode: '903',
  territoryWording: 'Green Card territory applies according to the Hellenic Motor Insurers Bureau rules.',
};
...
const CONFIGS: Record<string, JurisdictionProductConfig> = {
  ...
  'GR/MOTOR': makeConfig({
    ...
    countryName: 'Greece',
    productCode: 'MOTOR',
    taxProfile: 'GR_MOTOR_TENANT_IPT_CURRENT',
    greenCard: GR_GREEN_CARD,
    bdxImport: {
      defaultCountry: 'Greece',
      defaultCity: 'Athens',
      defaultProvince: 'Attica',
      defaultPostcode: '00000',
      defaultPhone: '+302100000000',
      defaultVehicleLocation: 'Greece',
      defaultCountryOfRegistration: 'Greece',
      defaultLicenseIssuedIn: 'Greece',
      defaultNationality: 'Greece',
      migrationSourceCode: 'BDX_VOLANTE_GREECE_2026',
    },
  }),
  'ES/MOTOR': makeConfig({
    countryCode: 'ES',
