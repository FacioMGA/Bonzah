...
describe('Protected Resource Metadata (RFC 9728)', () => {
    ...
    it('per-mount operator variant narrows to operator scopes only', () => {
        ...
        expect(m.scopes_supported.every((s) => s.startsWith('operator.'))).toBe(true);
    });

    it('per-mount config variant narrows to configuration scopes only', () => {
        ...
        expect(m.scopes_supported.every((s) => s.startsWith('configuration.'))).toBe(true);
    });
