 ...
 * surface gate + `configuration:publish_sandbox` permission (issuing a
 * key that can call `publishToSandbox` itself requires that permission;
 * issuing read-only keys requires `configuration:draft`).
 *
