 ...
 * read/comm/analytics; `operator.mutate` is an optional per-key
 * opt-in (V2). A regression here is the spec breaking; this test
 * fails loudly.
 */
...
describe('OPERATOR_AGENT permission policy (ADR-0036 amendment #2 + ADR-0039)', () => {
    ...
    it('V2 exposes operator.mutate as the only optional per-key opt-in (ADR-0039 §4)', () => {
        expect(OPERATOR_AGENT_OPTIONAL).toEqual(['operator.mutate']);
    });
