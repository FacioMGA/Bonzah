...
import { resolveTravelAdminFee } from './data/travel-fee-bands.loader.js';
import {
  ADDON_KEYS,
  lookupSingleTripCoverMultiplier,
  lookupPriorClaimLoading,
  lookupTravelAddon,
  lookupTravelRate,
  lookupUnderwritingProfitLoading,
  resolveAgeBand,
  resolveDayBand,
  type AddonKey,
  type AddonConfig,
...
export function calculateTravelPremium(data: TravelQuoteData): { premium: PremiumCalculation; breakdown: TravelBreakdown; refer: boolean; reason?: string; declined: boolean; declineReason?: string } {
  ...
  const singleTripMultiplier = tripType === 'Single trip' ? lookupSingleTripCoverMultiplier(coverType) : 1;
  const rate = lookupTravelRate(plan, tripType, area, lookupCoverType, dayBand, ageBand);
  if (rate === null) {
    return {
      premium: {
        premium: 0, basis: 'HYBRID',
  ...
  for (const name of ADDON_KEYS) {
    ...
    const config = lookupTravelAddon(name);
  ...
  const claimsLoading =
    data.risk?.hasPreviousTravelClaim === true && data.risk.previousTravelClaimBand === 'up_to_500'
      ? Number((basePremium * lookupPriorClaimLoading().upTo500Rate).toFixed(2))
      : 0;
  if (claimsLoading > 0) {
    steps.push({
      ...
      factor: lookupPriorClaimLoading().upTo500Rate,
      inputs: { appliesTo: lookupPriorClaimLoading().appliesTo, base: basePremium, band: 'up_to_500', source: 'brit-travel.priorClaimLoading' },
    });
  }

  const uwProfitLoadingConfig = lookupUnderwritingProfitLoading();
  ...
  const adminFee = resolveTravelAdminFee(netPremium);
  if (adminFee > 0) {
    steps.push({
      id: 'travel.fee.admin',
      name: 'Admin fee',
