...
export function buildMotorDocViewModel(args: {
  ...
  const vehicle0 = {
    ...
    description_full: (() => {
      ...
      if (qd.engineSize) parts.push(`(${Number(qd.engineSize)} cc)`);
      const fuel = safeStr(qd.fuelType);
  ...
  return {
    ...
    greenCard: {
      authority: safeStr(brand?.greenCardAuthority) || safeStr(greenCardConfig?.bureauName),
      ...
      signature_asset: safeStr(brand?.uwSignatureAsset) || 'ps-signature.png',
    },
    lloyds: {
