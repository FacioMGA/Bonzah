...
type UnderwritingModule = typeof import('../underwriting/motorUwAutomation.js');
type PricingMatrixLoaderModule = typeof import('./data/loader.js');
type CanonicalRulesModule = typeof import('./canonicalRules.js');
type ClassicPricingModule = typeof import('./classicPricing.js');

const underwritingModule: UnderwritingModule = await import('../underwriting/motorUwAutomation.js');
const pricingMatrixLoaderModule: PricingMatrixLoaderModule = await import('./data/loader.js');
...
const abbeygateAutoCyprus2022 = pricingMatrixLoaderModule.loadAbbeygateAutoCyprus2022Matrix();
const { evaluateMotorUwAutomation } = underwritingModule;
const {
...
function parseExcessFromQuoteData(quoteData: QuoteData, overrideExcess?: number): number {
  ...
  const calculatedMin = computeCalculatedPolicyExcess(quoteData);
  if (isClassicRisk(quoteData.vehicleType)) return calculatedMin;
  if (!Number.isFinite(n) || n <= 0) return calculatedMin || 250;
  return Math.max(n, calculatedMin || 250);
}
...
export function calculateAutoInsuranceQuoteResponse(
  ...
  opts?: {
    reference?: string;
    currency?: string;
    uwDecision?: ReturnType<typeof evaluateMotorUwAutomation>;
    jurisdictionConfig?: JurisdictionProductConfig;
  },
  appliedEndorsements: Array<{ code: string; params?: unknown }> = [],
  matrix: typeof abbeygateAutoCyprus2022 = abbeygateAutoCyprus2022
): QuoteResponse {
  ...
  // 1. Run Underwriting Automation
  const uwDecision = opts?.uwDecision || evaluateMotorUwAutomation(normalizedQuoteData);
  ...
  const primaryExcess = parseExcessFromQuoteData(normalizedQuoteData, overrideExcess);
  ...
  const minExcess = computeCalculatedPolicyExcess(normalizedQuoteData);
  const altExcesses = buildHigherDeductibleOptions(minExcess, 1000).filter((x) => x !== primaryExcess);
...
export function calculateAutoInsurancePremium(
  ...
  const proposerAgeFactor = proposerAgeFactorFromScheme(age);
  const vehicleAgeFactor = vehicleAgeFactorFromMatrix(Number(quoteData.year || 0));
  const licencePeriodFactor = licencePeriodFactorFromScheme(effectiveLicenseYears);
  ...
  const twoNamedDriversDiscount = namedDriversDiscountFactor(driverPricingBasis);
  const openDriverAgeBandDiscount = openDriverAgeBandDiscountFactor(driverRestriction);
  const restrictionMinAge = driverRestrictionMinAge(driverRestriction);
  const useFactor = vehicleUseFactorFromScheme(String(normalizedQuoteData.vehicleUse || ''));
  const convictionFactor = convictionFactorFromScheme(normalizedQuoteData);
  const claimsFactor = claimsFactorFromScheme(normalizedQuoteData);
  const maxAddDriverAge = maxAdditionalDriverAge(normalizedQuoteData, getAge);
  const additionalDriversOver80Factor = Boolean(normalizedQuoteData.hasAdditionalDrivers) && maxAddDriverAge !== null && maxAddDriverAge > 80
    ? 1.2
    : 1.0;
  ...
  const baseExcess =
    isTpo ? 0
      : isClassic ? Math.max(Number(classicContext?.policyExcess || 0), computeClassicExcess(Number(vehicleValue || 0)))
        : isMotorbike ? motorcycleCompExcessFromWorkbook(Number(normalizedQuoteData.engineSize || 0))
          : isMotorcaravan ? computeMotorcaravanExcess(Number(vehicleValue || 0))
            : computeDeclaredValueBasedExcess(Number(vehicleValue || 0), Number(normalizedQuoteData.engineSize || 0));
  const excess = isTpo ? 0 : isClassic ? baseExcess : Math.max(baseExcess, Number(selectedExcess || 0));
  const referenceCompExcess = isTpo
    ? computeCalculatedPolicyExcess({ ...normalizedQuoteData, coverRequired: 'Comprehensive' })
    : excess;
  const excessOrigin = isTpo ? 'TPO fixed at €0'
    : isClassic ? 'Classic matrix rule (age band × vehicle group)'
      : isMotorcaravan ? 'Motorcaravan value threshold'
        : 'max(1% declared value, CC-band minimum), rounded up to €50';
  steps.push({
  ...
  // Policy term
  const { termMonths, termFactor } = resolveTermFactor(quoteDataExtra);
  ...
  if (termFactor !== 1.0) {
    steps.push({
      ...
      notes: 'Six-month policies are rated at 60% of annual premium.',
    });
  ...
  if (isTpo && !motorbikeWorkbookRated) {
    ...
    tplRaw = fromDecimal(roundCurrency(toDecimal(comprehensiveBaseRate).mul(0.6)));
    compRaw = 0;
    steps.push({
      ...
      factor: 0.6, output: tplRaw, notes: 'Applied before endorsement add-ons.',
    });
  }
  ...
  if (isMotorcaravan) {
    ...
    const overThreeTonnesFactor = weightTonnes > 3 ? 1.3 : 1.0;
    const regionRaw = String(quoteData.proposer?.address?.city || quoteData.vehicleLocation || '').toLowerCase();
    const limassolFactor = regionRaw.includes('limassol') ? 1.25 : 1.0;
    const loaded = fromDecimal(roundCurrency(toDecimal(base).mul(overThreeTonnesFactor).mul(limassolFactor)));
    tplRaw = 0;
    compRaw = loaded;
    ...
    if (limassolFactor !== 1.0) {
      steps.push({ id: 'motorcaravan.loading.location', name: 'Motorcaravan Limassol loading', kind: 'factor', inputs: { location: regionRaw || 'unknown' }, factor: limassolFactor });
    }
  }
  ...
  if (isClassic && classicContext) {
    steps.push({
      ...
      inputs: { age, ageBand: classicContext.ageBand, vehicleGroup: classicContext.vehicleGroup, groupMatchType: classicContext.groupMatchType, matrixExcess: classicContext.policyExcess, valuePctExcess: computeClassicExcess(Number(vehicleValue || 0)) },
  ...
  const ncdDiscountPct = isFlatSchemeRisk ? 0 : ncdDiscountPctFromScheme(normalizedQuoteData.ncb);
  const protectedNcdFactor = isFlatSchemeRisk ? 1.0 : protectedNcdPremiumFactor(Boolean(normalizedQuoteData.protectNCB), normalizedQuoteData.ncb);
  ...
  const protectedNcdAmount = protectedNcdFactor !== 1.0 ? fromDecimal(roundCurrency(toDecimal(grossPremium).mul(0.1))) : 0;
  ...
  const onlineDiscount = !isFlatSchemeRisk && shouldApplyOnlineDiscount(quoteDataExtra)
    ? fromDecimal(roundCurrency(toDecimal(technicalNet).mul(0.03)))
    : 0;
  ...
  const uwAdjustmentPctCap = resolveUwAdjustmentPctCap(normalizedQuoteData);
