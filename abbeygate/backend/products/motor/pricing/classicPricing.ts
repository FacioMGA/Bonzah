...
import { loadClassicCarRates } from './data/loader.js';
import type {
  ClassicCarRates,
  ClassicRateGroup,
  ClassicMileageBand,
...
function rates(): ClassicCarRates {
  return loadClassicCarRates();
}
...
export function resolveClassicVehicleGroup(input: {
  ...
  year: unknown;
  engineCc: unknown;
}): { vehicleGroup: ClassicVehicleGroup; groupMatchType: ClassicGroupMatchType } {
...
export function resolveClassicPricingContext(args: {
  ...
  const premiumRow = r.premiumByMileageBand[mileageBand] as Record<string, number>;
  const excessRow = r.policyExcessByAgeBand[ageBand] as Record<string, number>;
