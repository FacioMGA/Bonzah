...
async function resolvePublicAutoPolicyWithSelect<TSelect extends Prisma.PolicySelect>(
  sessionId: string,
  select: TSelect,
  perf?: PerfTimings
): Promise<{ policy: Prisma.PolicyGetPayload<{ select: TSelect }> | null; mode: PublicSessionResolveMode }> {
...
export async function resolvePublicAutoPolicyLight(
  ...
  try {
    ...
    return resolved as { policy: PolicyLight | null; mode: PublicSessionResolveMode };
  } finally {
    if (lock.acquired) {
...
export async function resolvePublicAutoPolicyFull(
  ...
  return resolved as { policy: PolicyFull | null; mode: PublicSessionResolveMode };
}
