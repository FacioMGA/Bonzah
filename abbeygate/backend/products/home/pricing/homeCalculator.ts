...
import {
  getHomeMinPremium,
  getHomeWorkbookRateBlock,
  lookupHomeCountryBaseLoading,
  lookupHomeUnderwritingProfitLoading,
  lookupHomeWildfireLoading,
  type PropertyUse,
...
const HOME_MIN_PREMIUM = getHomeMinPremium();
...
function propertyAgeDiscount(yearBuilt?: string): number {
  if (yearBuilt === '1990 or Later') return 0.15;
  if (yearBuilt === '1980 to 1989') return 0.10;
  return 0;
}

function ncbDiscount(ncb?: string): number {
  switch (ncb) {
    case '1 Year':
    case '1 Years': return 0.10;
    case '2 Year':
    case '2 Years': return 0.125;
    case '3 Year':
    case '3 Years': return 0.15;
    case '4 Year':
    case '4 Years': return 0.20;
    case '5+ Years': return 0.20;
    default: return 0;
  }
}

function increasedExcessDiscount(excess?: string): number {
  if (excess === '350 XS') return 0.05;
  if (excess === '750 XS') return 0.10;
  return 0;
}

function claimsLoading(claims?: string): number {
  if (claims === '1 claim < 1000') return 0.30;
  if (claims === '2 claims < 3000') return 0.55;
  if (claims === '3 claims or > 3000') return 1.20;
  return 0;
}

function greekPostcodeLoading(pc?: string): number {
  // Athens 10 & 11: No Quote Risk (handled in UW as decline). For other Greek postcodes: no loading.
  if (pc === 'kefalonia' || pc === 'zakinthos' || pc === 'lefkada' || pc === 'santorini') return 0.35;
  return 0;
}

function wildfireLoading(input: HomeQuoteData): { rate: number; classification: WildfireRiskClassification | null } {
  ...
  const rates = lookupHomeWildfireLoading();
  if (classification.tier === 'amber') return { rate: rates.amber, classification };
  if (classification.tier === 'yellow') return { rate: rates.yellow, classification };
  if (classification.tier === 'green') return { rate: rates.green, classification };
  return { rate: 0, classification };
}

export function calculateHomePremium(input: HomeQuoteData): { premium: PremiumCalculation; breakdown: HomeBreakdown; declined: boolean; declineReason?: string } {
  ...
  const block = getHomeWorkbookRateBlock(tenant.countryCode, use);
  ...
  const HOME_EUROP_ASSISTANCE_JURISDICTIONS = new Set(['CY', 'GR']);
  const europAssistanceAvailable = HOME_EUROP_ASSISTANCE_JURISDICTIONS.has(tenant.countryCode);
  ...
  const europAssistanceFee = europAssistanceSelected ? 12 : 0;
  ...
  const countryBaseLoad = lookupHomeCountryBaseLoading(tenant.countryCode);
  ...
  if (combustible) {
    loadingBreakdown.combustibleConstruction = 1.0; loadings += 1.0;
    steps.push({ id: 'home.loading.combustible', name: 'Combustible construction loading', kind: 'factor', factor: 2.0, notes: '+100%' });
  }
  if (input.propertyType === 'Static Caravan') {
    loadingBreakdown.staticCaravan = 0.25; loadings += 0.25;
    steps.push({ id: 'home.loading.staticCaravan', name: 'Static caravan loading', kind: 'factor', factor: 1.25, notes: '+25%' });
  }
  const claimsLoad = claimsLoading(input.previousClaims);
  if (claimsLoad > 0) {
  ...
  const pcLoad = greekPostcodeLoading(input.greekPostcode);
  if (pcLoad > 0) {
  ...
  const wildfire = wildfireLoading(input);
  if (wildfire.rate > 0) {
  ...
  const propertyAge = propertyAgeDiscount(input.yearBuilt);
  ...
  const alarmDiscount = isTrue(input.alarm) ? 0.10 : 0;
  ...
  const over45 = isTrue(input.proposerOver45) ? 0.05 : 0;
  const excess = increasedExcessDiscount(input.increasedExcess);
  ...
  const ncb = ncbDiscount(input.noClaimsDiscount);
  ...
  const discretionary = Math.max(0, Math.min(0.20, asNumber(input.discretionaryDiscount)));
  ...
  const uwProfitLoadingConfig = lookupHomeUnderwritingProfitLoading();
  ...
  if (taxes.minimumApplied) {
    steps.push({ id: 'home.minimumPremium', name: 'Minimum premium applied', kind: 'subtotal', output: taxes.grossPremium, notes: `floor ${HOME_MIN_PREMIUM}` });
  }
...
function applyHomeWorkbookTaxes(
  ...
  if (country === 'ES') {
    ...
    const tenantTaxes = applyTenantTaxes(getTenantConfig(), Math.max(netBeforeMinimum, HOME_MIN_PREMIUM));
