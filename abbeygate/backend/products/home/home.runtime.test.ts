...
describe('home buildQuoteResponse uses resolvedCoverageSet', () => {
  ...
  it.each([
    ...
    const response = await homeProductRuntimeConfig.buildQuoteResponse({
      ...baseQuote,
      coverage,
  ...
  it('does not let MBE selection remove or change product-owned solar cover', async () => {
    const withoutCoverage = await homeProductRuntimeConfig.buildQuoteResponse(baseQuote, {}, { resolvedCoverageSet: null });
    const withCoverage = await homeProductRuntimeConfig.buildQuoteResponse(baseQuote, {}, {
      resolvedCoverageSet: {
        productType: 'HOME',
