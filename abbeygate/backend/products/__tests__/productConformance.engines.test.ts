  ...
  it.each(matrix)(
    ...
    async ({ tenant, adapter }) => {
      await runWithOperatingTenant(tenant, async () => {
        ...
        // UW engine: outcome must be a valid decision lane under every jurisdiction.
...
// GR must match CY's enabled product set for every product EXCEPT HEALTH,
// which is intentionally gated pending regulatory sign-off (ADR-0053).
// ---------------------------------------------------------------------------
describe('Greece (GR) product parity with Cyprus (CY)', () => {
  ...
  it('GR supports every product CY supports, except HEALTH (gated by ADR-0053)', () => {
    ...
    const expectedForGr = cySupported.filter((p) => p !== 'HEALTH');
