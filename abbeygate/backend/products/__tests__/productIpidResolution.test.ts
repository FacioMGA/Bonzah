...
describe('product IPID resolution (quote-stage disclosure)', () => {
  it('travel resolves its single static IPID asset', () => {
    const asset = new TravelProductAdapter().resolveIpidAsset('CY');
    expect(asset?.filename).toBe('Travel_IPID.pdf');
  ...
  it('home resolves a territory-specific IPID for each live territory', () => {
    ...
    expect(home.resolveIpidAsset('GR')?.filename).toBe('Home_IPID_Lloyds_Greece.pdf');
  ...
  it('motor resolves the approved Santam Cyprus IPID and fails closed elsewhere', () => {
    ...
    expect(() => motor.resolveIpidAsset('PT')).toThrow(/MOTOR_IPID_NOT_CONFIGURED/);
  });
});
