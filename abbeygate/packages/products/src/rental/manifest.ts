...
export const rentalManifest: ProductManifest = {
  ...
  insuredObject: {
    ...
    fields: [
      ...
      { path: 'risk.vehicle.declaredValue', label: 'Declared value', type: 'currency', required: true },
    ],
  },
  questionnaire: {
    sections: [
      ...
      },
      {
        ...
        fields: [
          ...
          { path: 'risk.vehicle.declaredValue', label: 'Declared value', type: 'currency', required: true },
          { path: 'risk.vehicle.repairProfile', label: 'Repair-cost profile', type: 'text', required: true },
      ...
      },
      {
        ...
        order: 3,
        fields: [{ path: 'coverages', label: 'Selected coverages', type: 'multiselect', required: true }],
      },
    ],
  },
  summaryFields: {
    ...
    buildTitle: (data) => {
      ...
      return [vehicle.year, vehicle.make, vehicle.model].filter(Boolean).join(' ') || 'Rental vehicle';
    },
  },
  listColumns: {
    insured: { primaryPaths: ['risk.vehicle.make', 'risk.vehicle.model'], secondaryPaths: ['risk.vehicle.class'] },
    coverage: { primaryPaths: ['coverages'], secondaryPaths: ['risk.pickup.state'] },
  },
  coverageCatalog: [
    { code: 'CDW', label: 'Collision Damage Waiver', description: 'Primary rental-vehicle collision damage protection.', scope: 'POLICY', group: 'damage' },
    { code: 'RCLI', label: "Renter's Contingent Liability Insurance", description: 'Primary state-minimum liability protection.', scope: 'POLICY', group: 'liability' },
    { code: 'SLI', label: 'Supplemental Liability Insurance', description: 'Additional liability protection; requires RCLI.', scope: 'POLICY', group: 'liability' },
    { code: 'PAI_PEI', label: 'Personal Accident / Personal Effects', description: 'Accident medical and personal-effects protection.', scope: 'POLICY', group: 'personal' },
  ],
  uwConfigSchema: {
    groups: [
      {
        ...
        fields: [
          { path: 'rating.maxVehicleMultiplier', label: 'Maximum vehicle multiplier', type: 'number' },
