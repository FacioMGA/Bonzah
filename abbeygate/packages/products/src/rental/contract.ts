...
export type RentalDiscoveredCoverage = {
  code: RentalCoverageCode; label: string; available: boolean; unavailableReason: string | null;
  requires: RentalCoverageCode[]; limit: string; deductible: string; description: string;
  indicativeDailyPrice: number | null; indicativeTripPrice: number | null;
};

export type RentalCoverageDiscoveryResponse = {
  eligibility: { status: RentalQuoteStatus; explanation: string; ruleReferences: string[]; referralRequirements: string[] };
  ...
  currency: 'USD'; ruleVersion: string; warnings: string[]; demoStatus: 'DEMO BUILD';
};
