 ...
 *
 * GHS (General Healthcare System) beneficiaries get an outpatient +
 * repatriation extension at no extra premium — modelled as a wizard
...
export const healthManifest: ProductManifest = {
  ...
  listColumns: {
    ...
    coverage: {
      ...
      buildSecondary: (data) => {
        const ghs = (data.ghs && typeof data.ghs === 'object') ? data.ghs as JsonObject : {};
        return ghs.isBeneficiary === true ? 'GESY extension' : 'Inpatient only';
      },
      buildStartDate: (data) => {
  ...
  coverageCatalog: [
    ...
    { code: 'HEALTH-GHS-EXTENSION', label: 'GESY Outpatient + Repatriation Extension', scope: 'POLICY', group: 'extensions' },
